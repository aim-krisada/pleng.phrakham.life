...
## 3 · Wireframe (b) — SHELL / NAV (create-vs-find + minimal)
...
## 4 · Wireframe (c) — หน้าเพลง (ตัด 20+ ปุ่ม → แกนหลัก + progressive disclosure)
...
### US-G1 — หน้าแรกนำด้วยเพลง (แก้ S-2, S-3)
*ในฐานะ* คนร้องที่เปิดเว็บมาหาเพลง *ฉันต้องการ* เห็นเพลงทันทีโดยไม่ต้องเลือกเล่มก่อน *เพื่อ* เจอเพลงเร็วเหมือน Spotify/Apple Music
- **AC-G1.1** landing (`/`, query ว่าง) โชว์ **รายการเพลง (ตัวกรอง "ทั้งหมด")** เป็น default — ไม่ใช่กริดเล่มบังคับ.
- **AC-G1.2** มี chip 3 ตัว `ทั้งหมด · เล่ม · ★` · "เล่ม" = bookshelf เดิม (drill เข้าเพลงได้เหมือนเดิม · ไม่ regress US home-redesign).
- **AC-G1.3** placeholder ช่องค้น = สั้นเป็นภาษาคน ("ค้นหาเพลง") · ไม่ล้นที่ 360px (แก้ S-3 · แต่ syntax เดิม เลข/คีย์/โน้ต ยังค้นได้ — ไม่แตะ `songSearch.js`).
- **AC-G1.4** query มีค่า → ผลค้นหาแบน override ทุกตัวกรอง (คงพฤติกรรมเดิม) · ล้าง query → กลับตัวกรองก่อนหน้า.
- *test:* mount SongList → assert default = list ไม่ใช่ grid · สลับ chip เปลี่ยน view · axe 0 · no-hscroll @360/412.
