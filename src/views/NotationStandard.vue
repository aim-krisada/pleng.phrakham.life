...
<template>
  <div class="reading-page notation-page">
    ...
    <section class="card" id="roots" aria-labelledby="roots-h">
      ...
      <p>สัญลักษณ์ทุกตัวซ้อนกันได้ในโน้ตตัวเดียว — <strong>ลำดับมาตรฐาน</strong>คือ:</p>
      <p><code>[~] → [# / b / n] → [.] จุดต่ำ → ตัวเลข → ['] จุดสูง → [_] เส้นใต้ → [.] ประจุด → [~] → [^]</code></p>
      <p class="callout">
        <strong>พิมพ์สลับลำดับได้ ไม่พังแล้ว</strong> — ตัวแก้จะ<strong>จัดลำดับให้เอง</strong>ทันทีที่พิมพ์
        (พิมพ์ <code>5^.</code> ได้ <code>5.^</code> · พิมพ์ <code>.#4</code> ได้ <code>#.4</code> ·
        พิมพ์ <code>4#</code> ได้ <code>#4</code>) โดยช่องนั้นจะกะพริบให้เห็นว่าระบบจัดให้
        <strong>ตัวอักษรเท่าเดิม สลับที่เท่านั้น</strong> ไม่มีการเพิ่มหรือตัดอะไรทิ้ง
        <br />
        ยกเว้น 2 ตัวที่<strong>ตำแหน่งคือความหมาย</strong> จึงอยู่ฝั่งเดิมเสมอ:
        <code>.</code> หน้าเลข = จุดเสียงต่ำ · หลังเลข = จุดเพิ่มค่า ·
        <code>~</code> หน้าเลข = ปลายไท · หลังเลข = ต้นไท
        <br />
        <strong>เพลงเก่าในคลังที่เขียนสลับลำดับ ระบบจะอ่านให้ถูกแต่ไม่แก้ข้อมูลให้เอง</strong> —
        จะขึ้นข้อความบอกว่าเก็บไว้ว่าอะไรและอ่านเป็นอะไร ให้คนเทียบกับหนังสือต้นฉบับแล้วตัดสินเอง
      </p>
