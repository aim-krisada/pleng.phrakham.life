// Tests for notationLint. Run with the rest of the suite: `npm test`.
// Was a standalone `node` script calling process.exit() — vitest counted 0 cases from it and the
// stray exit made `npm test` fail even when everything passed. Converted to real vitest cases
// (assertions unchanged); `check()` is kept as a thin wrapper so every case reads as before.
