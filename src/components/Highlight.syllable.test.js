...
describe('SongSheet — per-syllable render, highlight and tap-to-jump', () => {
  ...
  // เนื้อล้วน DISPLAY mode (lyric shown, note+chord hidden): a v2 song still renders per-syllable
  // spans (inline, readable words) so the karaoke walks SYLLABLE-by-syllable, not whole-segment
  // (P'Aim 14 ก.ค. "อยากได้เป็นพยางค์ ไม่ใช่ท่อน").
  it('v2 in lyrics-only display → per-syllable spans + syl-playing (walks syllable-by-syllable)', () => {
    const w = mount(SongSheet, { props: { content, showChord: false, showNote: false, showLyric: true, playingSyl: { li: 0, si: 0, syk: 1 } } })
    expect(w.findAll('.syl').length).toBe(3) // per-syllable spans render even in lyrics-only
    const lit = w.findAll('.syl-playing')
    expect(lit.length).toBe(1)
