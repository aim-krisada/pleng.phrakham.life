...
- **หน้าแก้ไขล็อกเลือกได้เฉพาะ 3 เล่ม** (B095 · P'Aim 12 ก.ค. ผ่าน PM) — ช่อง "หมวด" ใน editor
  เสนอเฉพาะ 3 เล่ม canonical (เล่มใหญ่/อนุชน/เด็กเล็ก) · **ไม่เปิดให้พิมพ์ค่านอกลิสต์** (`allow-custom` OFF —
  ค่าที่พิมพ์นอกลิสต์ถูก reject ตอน blur) = data-integrity กันชื่อเล่มพิมพ์เพี้ยน
  · **DB column `category` ยัง free-text ทางเทคนิค** แต่ UI ไม่เปิดพิมพ์เอง · **เพิ่มเล่มที่ 4 = แก้ `CATEGORY_OPTIONS` 1 บรรทัด**
  (ใน `EditorMode.vue`) · ชื่อสวยเติมใน `bookshelf.js`; code ที่ไม่รู้จัก (ข้อมูลเก่า) ยังแสดงดิบ ๆ ไม่พัง
