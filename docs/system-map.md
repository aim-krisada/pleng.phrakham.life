...
- **`category` เป็น free-text ที่ระดับ DB** — เพิ่มเล่มใหม่ = ข้อมูลใส่ code ใหม่ได้ ไม่ต้องแก้ schema
  (ชื่อสวยเติมใน `bookshelf.js`; code ที่ไม่รู้จักก็ยังแสดงดิบ ๆ ไม่พัง) · **แต่หน้าแก้ไขล็อกให้เลือกได้
  เฉพาะ 3 เล่ม canonical** (B095 · ตัด allow-custom) เพื่อกันเล่มมั่ว — เพิ่มเล่มที่ 4 ต้องแก้ `CATEGORY_OPTIONS`
