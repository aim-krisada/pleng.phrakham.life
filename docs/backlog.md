...
| B004 | ปรับปรุง | print-polish (นอกไฟล์ WT-B) | 3 งานจาก handoff WT-B: (1) ชื่อไฟล์ PDF = ชื่อเพลง (เซ็ต `document.title` ตอนพิมพ์) (2) ส่ง `:song-title` เข้า `SongSheet` footer (3) "หน้า X ของ Y" + margin กันเนื้อทับ footer ผ่าน `@page` ใน `src/styles.css` · snippet ครบใน `docs/reports/wt-b-print.md` · **เจ้าของ = WT-0** (ปุ่มพิมพ์อยู่โหมดแผ่น = `Studio.vue` + `styles.css`) · ทำรอบ print-polish | in-US | US-I2 + US-I3 |
| B003 | ปรับปรุง | ตัดเมนู "เลือกเพลงเพื่อแก้…" ที่ซ้ำในโหมดแก้ | หลัง US-05 มีปุ่ม "เปิดเพลง" ที่ shell ทุกโหมดแล้ว → เมนูของ `EditorMode` ซ้ำ · ตัดออกตอน WT-D แตะ `EditorMode.vue` (ไม่กระทบการใช้งาน โหลดได้ทั้งสองทาง) | in-US | US-I5 (รอ WT-D) |
