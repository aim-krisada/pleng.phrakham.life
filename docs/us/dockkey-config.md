...
| 2 | resize *ขนาด* dock (P'Aim: **free-form ไม่ใช่ preset**) | ✅ **desktop = ลากขอบต่อเนื่อง · mobile = สไลเดอร์ขนาด** (free-form ต่อเนื่องเหมือนกัน · touch-safe · reuse แพทเทิร์นสไลเดอร์โปร่งใสเดิม `:386`) · พื้นขั้นต่ำ 44px | pointer drag (desktop) · slider (touch · SOP §1 ห้าม drag-resize mobile) · WCAG 44px floor |
...
> ### 🔴 SCOPE ที่ทำจริง (PM/P'Aim 18 ก.ค. · ตีกลับ mockup v2)
> **P'Aim: หน้าตั้งค่า ⚙ เดิม (วนซ้ำ/คอร์ด/ความเร็ว/แสดงผล/โปร่งใส/ปัก) "ดีอยู่แล้ว — ห้ามแตะ"** · PM verify โค้ดจริง: ปัก/เลื่อน ▲▼/settings/collapse/ลากย้าย **มีครบแล้ว** (ผมเผลอเสนอซ้ำ = ผิด SOP §4 ของตัวเอง) · **DELTA จริงมีแค่ 2:**
> 1. **ปรับ *ความกว้าง* dock → ปุ่ม reflow** (แบบ 2 · ไม่ใช่ซูม) — desktop = ลากขอบ · mobile = **สไลเดอร์ "ความกว้างแถบ"** (ฟันธง: touch drag เล็งยาก SOP §1 · reuse pattern สไลเดอร์เดิม) · ปุ่มคง 44px
> 2. **มือถือ: ⚙ เปิดหน้า *เต็มจอ*** แทน popover เล็ก (`dk-pop dk-panel` คับบน 360px) — ของเดิมครบ + สไลเดอร์ขนาด · เลือกเสร็จกลับจุดเดิม
>
...
**⭐ "อิสระ" = แบบ 2 (free-form window resize + REFLOW) ไม่ใช่ uniform zoom** (P'Aim ยืนยันผ่าน PM 18 ก.ค.):
- ❌ **แบบ 1 (v3 ทำผิด) = ซูมทั้งก้อน** (`--scale` โต/หดสัดส่วนเดิม) — **ทิ้ง**
- ✅ **แบบ 2 = คุม *ความกว้าง* แถบ → ปุ่ม reflow** (กว้างขึ้น=ปุ่ม/แถวมากขึ้น·แถวน้อยลง · แคบลง=ขึ้นบรรทัดใหม่) · **ปุ่มขนาดคงที่ 44px เสมอ**
- **engine เดิมทำ reflow อยู่แล้ว** (`buildRows`/`cap`/`flex-wrap` + `width:fit-content;max-width:min(700px,100vw-16px)`) → resize = ให้ผู้ใช้คุม *ความกว้าง/footprint* แล้วปล่อย engine repack · height = ผลจากจำนวนแถว

| platform | ท่า (input) | ผล |
|---|---|---|
| **desktop (เมาส์)** | **ลากขอบ dock** ตั้งความกว้าง | engine reflow ปุ่มตามความกว้าง |
| **มือถือ (นิ้ว)** | **สไลเดอร์ "ความกว้างแถบ"** ในหน้าตั้งค่าเต็มจอ (⛔ ไม่ใช่ซูม · ไม่ใช่ drag บน touch SOP §1) | คุมความกว้าง→reflow เหมือนกัน · touch-safe |
| ทั้งคู่ | — | **ปุ่มไม่เล็กกว่า floor จริง (44/42/40px)** · footprint เล็กลง = ปุ่มต่อแถวน้อยลง/ซ่อนปุ่ม ไม่ใช่ย่อ target |

- **ขนาดคุมทั้งชุด** (ปุ่ม+gap+padding scale พร้อมกัน) → footprint เล็ก/ใหญ่ต่อเนื่องจริง
- **desktop = ลากขอบ** (ต่อยอด drag/pointer primitive เดิม) · **mobile = สไลเดอร์** ในหน้าเต็มจอ (§3.1)
- ทั้งสอง = free-form ต่อเนื่อง (align up · ถูกท่าต่อ platform) · min 44px
