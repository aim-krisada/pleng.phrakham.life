...
- 🧪 **[Sound Lab — โหมดทดสอบเสียงในแอป](backlog-sound-lab.md)** (P'Aim 18 ก.ค. · backlog) — ยกหน้า `cello-listen.html` เป็นฟีเจอร์ในแอปให้คนทำเสียงลองเทคนิค+จูนเองได้ทุกเพลง · ต้นแบบมีครบแล้ว · งาน integration+สิทธิ์=PM/UX/dev · **แจ้ง PM แล้ว**
...
---
## Log
- **2026-07-18 (ต่อ8):** P Aim: cello เดี่ยว+duo "ไม่เพราะ · ไม่มีลูกเล่น/คุยกันเหมือน Canon" → surface reference-track-analysis (Canon+ATY วัดแล้ว): เพราะ=เส้นเดินทาง 13-15dB + หายใจร่วม + balance +1dB(ไม่ใช่ counter-melody) → **แก้ balance เชลโลกลบเปียโน (+17→+3dB · baseMakeup×0.2)** + **spike เส้นเดินทางอารมณ์ (arc knob เดิม) + call-response ใหม่ (trading knob · แบ่ง melody เป็นวรรค phrase(rest OR 8beat) สลับ cello/piano)** วัด: cello 30→21→15 โน้ต@0/.5/1 · P Aim ขับด้วยหู
- **2026-07-18 (ต่อ7):** G ให้ 3 ทิศเสียง (ทำแล้ว · ไม่แตะ UI ตาม P Aim): (a) ไม่หา sample ใหม่/ไม่ formant-preserve (b) tail-darken หน่วง 0.3วิ (c) **chamber convolution reverb** (spike-local · predelay+early-refl+LP tail · knob ระยะห่าง default off · meter subtle -0.3dB = perceptual) · + P Aim: #2 ของขวัญ วิ4.6 "เสียงโดด" = midi69 ดังโดด +6dB (level ไม่ใช่ brightness · arranger accent เด่นบนเชลโล) → **DynamicsCompressor บนบัสเชลโล (celloEven · default 0.5)** วัด: pop 6.0→4.8(@.5)→2.4(@.8)dB · + **song picker (48 เพลง) + toggle ทั้งเพลง** ให้ audition หลายเพลง/ยาว
- **2026-07-18 (ต่อ6):** P'Aim ตั้งคำถามสำคัญ: อย่าแก้ manual เป็นจุด · ต้องเป็น **สมการกลางใช้ได้ทุกเพลงเหมือน golden piano** · ยอมปล่อยนิดๆ ได้ · **เขียนสรุปเทคนิคทั้งหมด `docs/reports/cello-tech-summary.md`** ให้ปรึกษา G · ชี้แจง: กฎที่ใส่เป็นสมการกลางหมด (key ที่ shift/length/level ต่อโน้ต ไม่เจาะจงเพลง) · จุด empirical สุด = tail 1.5× (flag ให้ G)
- **2026-07-18 (ต่อ5):** default(8) สม่ำเสมอขึ้น "นอกนั้นนุ่มมาก" เหลือ "แหบสั้นๆ วิ7.8" · วัด: vibrato ไม่เกี่ยว (vib0==vib22) · เป็น**โน้ตยาว G#4 (ดึงลง) หางโน้ตสว่างขึ้นเอง** (bloom ของ p-bow · 2วิ ไต่ ~4dB) → เพิ่ม tail-darkening: โน้ตยาว(>1วิ)+ถูกดึงเสียง ramp cut ไป 1.5×knob ที่หางโน้ต + down-shift weight 0.5→0.6 · วัด: วิ7.8 −4.1→−5.5 (เท่าโน้ตทุ้ม −5.4) · exact-pitch ไม่แตะ
- **2026-07-18 (ต่อ4):** พี่เอมได้ยิน "แหบ↔ทุ้ม สลับเป็นตัวๆ" → วัดยืนยัน = artifact ของ **pitch-shift จาก 17 ระดับเสียงของ Karoryfer** (ดึงขึ้น=แหบ/สว่าง +1→−2.5dB · ตรงไฟล์=ทุ้ม/อุ่น 0→−5.4dB · ดึงลง −1→−3.9dB) · หูพี่เอมตรง 4/4 จุด (วิ5.2/16=ตรงไฟล์=ทุ้ม · ก่อน5/วิ14=ดึงขึ้น=แหบ) → **เปลี่ยน treble tame จาก keyed-by-pitch เป็น keyed-by-shift** (ดึงขึ้นโดนเต็ม · ดึงลงครึ่ง · ตรงไฟล์ไม่แตะ) → วัด: ที่ 16 spread ลด 2.9→~1.4dB (แหบ −2.5→−4.4 เข้าหาทุ้ม) · knob = "ความสม่ำเสมอ"
- **2026-07-18 (ต่อ3):** พี่เอม max treble(20) ยัง "แสบนิดนึง" + arc→10dB ทำแสบกลับ → วัดเจอ shelf 2800Hz อยู่*เหนือ* harsh mid-harmonic (G#4 3rd-5th ~1.2-2.5kHz) เอื้อมไม่ถึง → ลด shelf 2200Hz + ramp 63→66 (harsh cluster เต็ม) + max 28 · วัด: >1.5kHz band ที่โน้ต harsh ลด 5.8dB@20 (เดิม ~1dB) = เอื้อมถึงแล้ว
- **2026-07-18 (ต่อ2):** พี่เอมลอง slider 3/8/14/16 "ยังแสบ" + "คุมไม่ได้ · เสียงตีกัน · ต้องมีปุ่มหยุด" → (1) player exclusive + ปุ่ม ⏹ หยุด + ปุ่มเล่นสลับ ▶/⏸ (2) **เจอบั๊ก ramp: TAME_LO/HI 62→74 ให้โน้ตแสบจริง (midi 66-73) โดนตัดแค่ 33-75% ของ slider → "16" ตัด midi-68 แค่ 8dB** · แก้เป็น 64→70 (โน้ตแสบโดนเต็ม · โน้ตต่ำ≤64 ไม่แตะ) · slider max→20 · **รอพี่เอมลองใหม่ + timestamp "กดแตร"** (สงสัย mid honk คนละแบนด์กับ treble — รอ timestamp วัด)
