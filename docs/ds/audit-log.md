...
## ✅ เคาะแล้ว — P'Aim (13 ก.ค. · ตรงจาก chat) · GO build
1. **จับ `song_drafts` ด้วย** — ✅ เพิ่ม (create/edit/submit/reject ฝั่งคนทำเพลง) ตาม §3
2. **op ดิบ → event มีความหมาย** — ✅ ต้อง "ดูง่าย สะท้อนความเป็นจริง" ตาม §3 (`create·edit·submit·approve_publish·reject·edit_published·unpublish`)
3. **RPC `approve_and_publish`** — ✅ **ทำเลย** (P'Aim อยากให้ "อนุมัติ+เผยแพร่" เห็นเป็น **1 เหตุการณ์ ใครกดชัด**) → ใช้ RPC + `op_group` ตาม §4
4. **`actor_name` snapshot (§1)** — ✅ **ต้องทำ (แก้ 13 ก.ค. · P'Aim ยืนยันผ่าน PM — กลับคำจากที่ SA session สรุปไว้เดิม)** — เก็บ **สำเนาชื่อ ณ ตอนทำ** ลงคอลัมน์ `actor_name` (+`actor_role`) ทุก event เพื่อให้ประวัติเก่าอ่านได้แม้ user ถูกลบ/เปลี่ยนชื่อ → **ชื่อต้องไม่หาย** · trigger/RPC ต้อง snapshot ชื่อจาก `profiles` ลง log ตอนเขียน (ไม่ใช่ map ตอนแสดง) · schema §1 (มี `actor_name` อยู่แล้ว) = ถูก · **fk `actor_id` คงไว้ด้วย** (ไว้ join เพิ่มเติม)
5. **วิวัฒน์ `song_revisions` ในที่เดิม** — ✅ (ไม่ทำตารางใหม่ · data เดิมไม่หาย)
6. **"ถอน" vs "ลบ"** — คง `unpublish` เดียวไปก่อน (สะท้อนจริง = เพลงออกจากคลัง) · เพิ่ม temp-unpublish ทีหลังได้ถ้าต้องการ
7. **ใครเห็น log (ประเด็นใหม่จาก P'Aim)** — ✅ **logged-in เห็นได้หมดทุกคน ไม่แยกตาม role.** SA ยืนยันผ่านมาตรฐาน (ISO 27001 A.12.4): หัวใจ audit = **แก้/ลบ log ไม่ได้** (§5 ทำแล้ว) ไม่ใช่ "ใครอ่านได้" · เนื้อ log = เนื้อเพลง (สาธารณะ) + ชื่อทีม ไม่มี PII/รหัส · ตรงกับ RLS เดิม `song_revisions` ("authenticated อ่านได้") · role มีแค่ editor/approver ต่างกันแค่สิทธิ์เขียน ไม่มีเหตุกั้นสิทธิ์อ่าน → คง SELECT ให้ผู้ล็อกอินทุกคน (§5 คงเดิม ไม่ต้องเพิ่ม view แยก role)

**เหตุเร่ง:** จะเริ่มมีคนแก้เพลงหลายคนเร็ว ๆ นี้ → build ก่อนคนเข้าเยอะ เพื่อไม่ให้ช่วง drafts หลุด log
