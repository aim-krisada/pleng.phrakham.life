...
**✅ LOCKED — เต็มวง Option 1 (P'Aim sign-off 13 ก.ค. "ดีพอสำหรับ 1st release") · dev implement เข้า scheduler แทนสูตรเต็มวงเดิม:**
ต้นเหตุ "จืด+เหมือนออร์แกน" = ไวโอลิน solo ถูกลากยาวเป็น pad. แก้โดยเปลี่ยนไวโอลินจาก pad → **นักดนตรีคนที่ 2 ที่โต้ตอบ**. เต็มวง = **เปียโนทริโอ + ไวโอลินสอดรับ** (ref `docs/spikes/ensemble-callresp-demo.html`).
1. **ตัด pad สายค้างหมด** — ไม่มี role สายลากคอร์ดพื้นหลังเลย. เต็มวง 3 เสียง: เปียโน (ทำนอง + arpeggio motion, front/near-reverb) · เชลโลเบส (re-bow ~3 บีต, far-reverb หลัง) · ไวโอลินสอดรับ (far-reverb หลัง).
2. **Call-and-Response (`violinFill`)** — ไวโอลิน**เงียบตอนเปียโนเดินทำนอง**. detector: โน้ตทำนอง **b ≥ 2.5 = "ช่องว่าง" ปลายวรรค** → ไวโอลินเล่นลูกเล่นสั้น **3 โน้ต** (turn สลับขึ้น/ลงตาม index) ในครึ่งหลังของช่องว่าง (เข้า `gapStart + max(0.75, gapBeats−2.0)`). โน้ต = chord tones ย่าน **MIDI 71–86** (เหนือทำนองเปียโน top≈73 → กันย่านชน). **ไม่มีใน verse ก็ได้** — เดโมเปิดทั้งเพลง P'Aim รับได้.
3. **Countermelody (`violinCounter`) — เฉพาะ chorus** — ไวโอลินเล่นเส้นประสาน: เข้า **offbeat** (`chordStart + 1.0` บีต) · ย่าน **74–86** (เหนือเปียโน) · คอร์ดยาว (≥3 บีต) = 2 โน้ตขยับ (เป็นเส้น ไม่ใช่ pad) · มีที่พักก่อนคอร์ดถัดไป · **ตัด counter ทิ้งบนคอร์ดที่มี fill อยู่แล้ว** (คุยกันชัด).
- **★ Balance principle (วัดจริง AnalyserNode ต่อ role · P'Aim เคาะด้วยหู):** เสียง "สอดรับ" ที่เป็น bowed **ดังกว่าตัวเลข peak** (sustain + vibrato + ย่านสูงใส หูไว) → ต้องตั้งให้ peak อยู่ **~10–13 dB ใต้เปียโนนำ** (ลึกกว่า 3–6 dB ที่ใช้กับเปียโนตัวที่สอง) · **countermelody ดัก (duck) ลึกกว่า gap-fill** เพราะเล่นทับ lead.
- **ค่าที่ล็อก (gain ก่อนคูณ section/velocity · CC baked +9/+10dB แล้ว → gain ต่ำ):** **fill = 0.21** · **counter = 0.14** · วัดได้ **เปียโน −7.3 dB · ไวโอลิน −16.6 dB (9.3 dB ใต้ lead) · เชลโล −23.1 dB**. section dynamics คงไว้ (verse ×0.7 → chorus ×1.0). ให้มี **สไลเดอร์ไวโอลิน** ผู้ใช้/tuning ปรับได้.
- **default เปิดแอป = รวมวง เปียโนนำ** (คงเดิม) · **โหมดไวโอลินนำ** (`ensemble-real-demo.html`) = ทางเลือก คงไว้.
