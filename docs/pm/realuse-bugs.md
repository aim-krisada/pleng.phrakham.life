...
| B038 | auto-scroll ตอนฝึกร้องไม่ตรงพยางค์ (เลื่อนไปหาท่อน) | ฝึกร้อง | ✅ บั๊กจริง — scroll watch `playingSeg` (ท่อน) ไม่ใช่ `playingSyl` (พยางค์); ตัวไฮไลต์ track พยางค์ถูกแล้ว | `SongViewer.vue` (watcher บรรทัด ~120-128) | G3 (highlight) | 🚚 **จ่าย H-highlight** |
