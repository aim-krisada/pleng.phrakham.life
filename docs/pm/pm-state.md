...
|---|---|---|
| `/` (v1) | **`1a55189`** | tag **`v1-frozen`** (branch `v1`) ← **ปล่อยรอบปิด v1 แล้ว 27 ก.ค.** |
| `/v2/` | **`27978bf`** | branch **`main`** (ไม่เปลี่ยน — commit `cebf366` บน main = empty commit ไว้จุด workflow เท่านั้น) |
...
**⚠️ ยังไม่มีใครฟังด้วยหูจริงสักคน** (ทั้งเพลง 33 และ 3 โหมดเปียโน) — ปล่อยไปโดยรู้ตัว ตามหลัก ship-fast · **ถ้าพี่เปาทักเรื่องเสียง = เรื่องนี้**
...
1. **รอฟีดแบ็กพี่เปาจากของที่เพิ่งปล่อย** — โดยเฉพาะ **เสียงเปียโน 3 โหมด "เพราะไหม"** (ไม่มีใครฟังก่อนปล่อย)
2. **จูนเสียงเปียโนให้ดีที่สุด** = งานหลักถัดไปตามทิศทาง P'Aim · แล้วค่อย cello · **แล้วค่อยเปิดเต็มวงกลับ** (flip `PIANO_ONLY = false` ใน `src/lib/soundOptions.js`)
3. **/v2 ยังไม่ได้รับ 2 fix ที่ v1 ได้ไป** — branch พร้อม merge: `claude/priceless-shtern-3ba8a1`@`9bd7769` (กันงานหาย) · `claude/quirky-grothendieck-75af84`@`5e2d307` (เปียโนเดี่ยว) · ⚠️ merge เข้า `main` = **ขึ้นเว็บทันที** ต้องขอ go ก่อน
4. **ถามพี่เปา: เพลงซ้ำ 2 คู่ในเล่มเด็กเล็ก** → แล้วรัน `db/011`
5. **รื้อ v2** — P'Aim สั่งว่าค่อยดูหลัง v1 เสร็จจริง · ตอนนี้ v1 ปล่อยแล้ว = เริ่มคุยได้ **แต่ต้องให้เขาเปิดเรื่องเอง**
