...
- **✅ P'Aim ยกระดับ: DESIGN อย่างเดียว ⛔ ไม่ลงโค้ด · ยกเป็น "ระบบวนร้อง/นำทางครบชุด"** (session `local_73ac8a99`) — สโคป `docs/ds/repeat-jumps.md`:
  ...
  - **🔴 3 decisions:** (2) al-target `to` field = **PM รับแล้ว** · **(1) Vamp scope + (3) line-level v1 = รอ P'Aim เคาะ** · build จริง = session ใหม่หลัง review + คิว hot-file · session wind down (จวนเต็ม)
