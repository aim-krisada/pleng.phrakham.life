...
4. ✅ **เพลงซ้ำปิดครบแล้ว 27 ก.ค.** — ลบ 4 แถว (เก็บ `bc7d1664` bpm 117 · `89355e79` bpm 92) · สำรองใน `public.songs_deleted_backup` · **`db/011` รันแล้ว** = unique index `songs_one_title_per_book` + คอลัมน์ `duplicate_ok` ลงจริง (PM ยืนยันผ่าน REST) (ตัวกันระดับฐานข้อมูล · **ยังไม่เคยรัน** · รันไม่ได้จนกว่า 2 คู่จะเคลียร์)
