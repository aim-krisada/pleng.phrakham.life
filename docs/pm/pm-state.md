...
### 🆕 พี่เอมใช้พรีวิวจริง 24 ก.ค. → editor flow polish (จ่าย SA spec แล้ว · G บังคับ · dev สร้างหลัง C)
1. **auto-scroll caret เข้าจอ** (ลูกศรขึ้น/ลงหลุดจอ ต้องเลื่อนเอง) 2. **ซ่อนแท็บบนสุดตอนโหมดแก้** (ต่อจากสาย A ที่ทำแท็บออกจากโหมดแก้) 3. 🎯 **คอร์ด = พิมพ์ตรงจุดที่กดเหมือนโน้ต ไม่ใช่ในแถบ** — **PM ฟันธงเห็นด้วย** (ตรงดีไซน์ล็อก+MuseScore chord-at-cursor · มีป๊อปอัปคอร์ดที่จุดโน้ตอยู่แล้ว ทำเป็นทางหลัก) · ระวังมือถือแป้นบังป๊อปอัป · spec = `docs/ds/editor-flow-polish.md`
