...
- 🎯 **เฟส POLISH (P'Aim สั่ง · ทุกงาน world-class + G consult บังคับ ผ่าน ai-bridge Chrome :9335):**
  ...
  - 🔴 **slur `( )` ลบไม่ได้ = รวมกลับเข้า BI-011 session `567d43e1`** (มี G context) → PM GATE เอา **(a) smart-span** (เลือกโน้ตไหนใน slur ก็ลบได้ · แก้ engine withToggledBox) + **(c) active-marks chip** [เอื้อน ✕] · ข้าม (b) แตะเส้น (Fitts trap ตาม G) · G วิเคราะห์: P'Aim ตันเพราะเลือกโน้ต**กลาง** slur = ไม่มีปุ่มติด · **verify slur JSON จริง คราวนี้** · ~~BI-011b `e1ba736f`~~ stood down (ซ้ำ · P'Aim กดเปิดแล้ว สั่งหยุด)
  ...
  - 🟡 **BI-012 Phase2 BUILD เสร็จ** `bi012-chord-entry @e0bf9e7` — คลิกช่องคอร์ดเปิด + ＋ affordance (discoverability) + Space/Tab รัวข้าม chord-cell ทั้งเพลง + soft-mark (ไม่เขียน+preview เตือน · ต่างจาก gate "เก็บ text" · ให้ Tester เช็ค silent ไหม) + pill preview + split ≥768 (เลี่ยง @media(hover)) · 1577 tests · HOT SongViewer.vue+SongSheet.vue · **verify ด้วย JS-event (pane hidden) → 🧪 Tester เครื่องจริง `task_1e9adc77`** (เมาส์/คีย์จริง+Surface+มือถือ OSK) ก่อน gate · builder wound down (re-dispatch ถ้า Tester เจอ)
  ...
  - ✅ **BI-018 A/B ไม่ชัด (design)** — chip `task_10ca8f82` analyze+G→STOP gate (multi-melody labeling/navigation world-class · ไม่รู้บรรทัด/outline เป็นของทำนองไหน) · ⚠️ HOT StructureDrawer.vue
