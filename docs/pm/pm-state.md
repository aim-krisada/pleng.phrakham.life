...
1. **auto-scroll caret เข้าจอ** (ลูกศรขึ้น/ลงหลุดจอ ต้องเลื่อนเอง) 2. **ซ่อนแท็บบนสุดตอนโหมดแก้** (ต่อจากสาย A) 3. 🎯 **คอร์ด = พิมพ์ตรงจุดที่กดเหมือนโน้ต ไม่ใช่ในแถบ** — **PM ฟันธงเห็นด้วย** (ตรงดีไซน์ล็อก+MuseScore chord-at-cursor · มีป๊อปอัปคอร์ดที่จุดโน้ตแล้ว ทำเป็นทางหลัก) · ระวังมือถือแป้นบังป๊อปอัป · 4. **เมนู "เพลง ▾" (dropdown สร้าง/เปิดเพลง ในหน้าเพลง)** = จ่าย Explore วิเคราะห์ว่าซ้ำหน้าแรก/ควรยุบเป็น palette ไหม (คืนข้อสรุปให้ PM) → รวมกับ 1-3 เป็นใบ spec เดียว `docs/ds/editor-flow-polish.md` ตอนจ่าย session ใหม่
