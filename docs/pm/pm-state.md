...
| **คอร์ด: รองรับทุกคอร์ดมาตรฐาน (พี่เปา · hotfix)** | 🔨 จ่ายแล้ว (spun · branch `chords-all-standard` จาก **main**) | engine พร้อม (parseChord) · แก้ input · → deploy main (P'Aim go) + port base |
...
- [ ] pre-merge editor (จ่าย editor แล้ว · จบแล้ว Tester verify → PM merge + auto-housekeep):
  - แก้ artifact `"` (เพลง `c71cbe9a`)
  - **#5 false-positive** (ทรงเป็นอยู่ในข้า `5f7fb5d2`): เส้น "แห่ง"→โน้ต 2 เกิน (P'Aim ตัดสิน) — investigate data/logic · ห้ามลบเส้นที่ถูกใน 1-3
  - **#4 dedup** (ทางแคบ `62355717`): derive วาดทับ `()` ที่ encode แล้ว = 2 เส้นซ้อน → ถ้ามี explicit `()` ห้าม derive ซ้ำ
