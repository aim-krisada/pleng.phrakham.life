...
| **B072 MP3 download** `mp3-download` | worktree `../pleng-mp3` (ยังไม่ push) | ✅ เสร็จ+ทดลองผ่าน (P'Aim มือถือ+PC) — client-side MP3 (OfflineAudioContext + lamejs · `scheduleNote` เสียง=ปุ่มฟัง) + **progress feedback รอบ2** (estimate ล่วงหน้า ยาว/ขนาด · progress bar+ETA · staged render→encode→done · yield main thread) · **verify เพลง100: pitch 201/201=100% · est error 0.025% · MP3 2.54MB/166s เล่นได้จริง PASS ✅** · เทสต์ 276 ผ่าน (+8) · build ผ่าน bundle เท่าเดิม (DownloadTool ยัง orphan) · report `docs/reports/mp3-download.md` · **🔔 รอ PM เคาะตำแหน่ง UI** → `docs/pm/proposal-mp3-placement.md` (เสนอ: รวมเมนู download เดียวกับ PDF/JSON · เสียบ dock ผ่าน descriptor สาย `sa-dockkey-print-edit`) |
