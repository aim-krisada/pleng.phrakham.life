...
|---|---|---|---|
| dock-core `wt-dock` | `71de90a` | ✅ **รอบ 2 เสร็จ · PM re-verified ครบ — พร้อม merge (รอ P'Aim accept รอบสุดท้าย)** · test 113/113 · build · แก้ 5 จุด: (1) ปุ่มลอยอยู่จุดที่กด ✅ (2) WCAG contrast 7.1:1 ✅ (3) ⋯ ขวาสุด ✅ (4) **transparency PM ตรวจเองในเบราว์เซอร์ = ทำงาน (α 0.92→0.58 ตอนขยับ slider) → ที่ P'Aim เห็นพัง = บิลด์เก่า/cache** ✅ (5) "แสดงผล" โชว์ badge + dock กว้าง 700 ✅ · **merge นี้ = gate ปลดล็อก B038/39/41** · ⚠️ merge มี doc conflict (backlog/board/brief-dock-core · wt-dock fork ก่อน PM commits) → PM เคลียร์เอง | debug pl2 r1 ✅ |
| B043 SA `wt-b043-sa` | (docs) | 🔄 design กำลัง iterate — folded PM #4 (music dock) + **selection model ใหม่ = Gmail-style** (click select/unselect · all/none · transport ตาม selection · ไม่เลือก=play all · ตอบ "ฝึกท่อนเดียว" ในตัว) · transport = **core reusable** เสียบ dock ผ่าน D8 · play/pause ไม่มี bg · **decision โต 6→8** (A แผ่เต็ม/ย่อ สำคัญสุด · +G ปุ่มนอก ⋯ · +H marker vs scrub) · **⚠️ dev ต้องหลัง dock-core merge + ประสาน** (แตะ SongViewer+StudioDock+DownloadTool) · รอ P'Aim เปิด wireframe เคาะ | debug pl2 r1 |
