...
| **B043 dev `wt-b043-dev`** | (committed) | ✅ **เฟส 1 เสร็จ — รอ PM ตรวจ+merge** · report `docs/reports/wt-b043-dev.md` · **Network URL http://10.215.141.98:5323/** · music dock ครบ: transport (progress+marker+⏮▶⏭🔁+scrub+แตะจุด) + ⚙ settings panel (ทุก control inline + 📌pin persist) + selector (Gmail list+All/None · C=ไม่จำ) สร้างเป็น **core ผ่าน D8 region:'top'** (StudioDock เพิ่ม top-region ใหม่ + `SingTransport.vue` core reusable · ไม่ฝังใน SongViewer) · ตัด `.section-bar`/การ์ดบนออก · download เข้า panel · **B038** scroll เล็ง `[data-syl]` · **B042** ▶/⏸ ต่อ+⏮ กลับต้น · play/pause icon-only · F=เงียบ (v1) · engine additive (`effectiveOrder`/`buildPlayNotes`/`playSong order` · ไม่แตะจังหวะ/ทำนอง) · **test 134/134 · build ✅ · พิสูจน์เล่นจริง** (posIndex/dot/ไฮไลต์พยางค์เดินจริง · ⏮⏭ ตรง marker boundary · scrub 50% · selection→order 273→133 · ⚙ inline+pin) · **ค้างให้ P'Aim มือถือ:** auto-scroll ตามพยางค์ (preview scroll แบบ programmatic ไม่ได้ในเครื่องนี้ · unit ยืนยัน target=`[data-syl]` + ไฮไลต์เดินตรงพยางค์) · **เฟส 2** (A2 แผ่นย่อ+verify print PDF) รอ merge ก่อน · **รอบ 2 (P'Aim art-direct):** layout เครื่องเล่นเพลง 2 แถวชัด (แถวบน grip+progress+เวลา · แถวล่าง ⚙ซ้ายสุด+⏮▶⏭🔁+ปุ่มปัก) · **default pin คีย์/ความเร็ว/แสดงผล** (แก้บาร์โล่ง) · **ความโปร่งย้ายเข้าแผง ⚙ แล้ว** (StudioDock ส่ง hook dock-alpha/dock-collapse ให้ top-region · dock ยัง generic) · หุบ→grip เครื่องเล่น · **รอบ 3 (P'Aim FINAL):** (A) **popup clamp ไม่หลุดขอบ** (ยก dock-polish clamp มาใช้แผง ⚙/dropdown/selector · verify จอสูง 300 แผงล้นบน→clamp เข้าจอ) · (B) ลำดับปุ่ม ⚙→⏮▶⏭🔁→คีย์/ความเร็ว/แสดงผล (แก้ pin order) · (C) **font ย้ายออกจาก dock → ปุ่ม "Aa" ใน top nav** (ShellBar · popup A↓[%]A↑+reset แบบ phrakham · fontScale global ใน store · +`FontTool.vue`) · test 142/142 · verify ครบ · **รอบ 4 (P'Aim ลอง 5323 · 5 จุด):** (1) grip ออกนอกกรอบ timeline (2) ✅**drag fix** (StudioDock ส่ง grip-down/move/up ให้ top-region · ลาก=ย้าย dock แตะ=หุบ · **พิสูจน์ลากจริง dock ขยับ 140,90**) (3) key→dropdown (4) ปุ่ม content-size (music dock อยู่แล้ว · ถ้ามีเจาะจงขอ P'Aim ชี้) (5) dropdown fit-content (`.mp-dd`+`.sd-pop-menu` max-content+nowrap · tempo ไม่ตัดบรรทัด · verify single-line+on-screen) · test 142/142 · build ✅ · **รอบ 5 (P'Aim ลอง 5323 เพลงมีท่อน · 4 จุด):** (1) ✅**collapse-in-place** (grip ไม่กระโดด · FAB วางที่ grip · พิสูจน์ center เท่ากันเป๊ะ) (2) ✅**dock fit-content** (`.sd-fit` · กว้างเท่าแถวปุ่ม · verify 488<700 · มือถือคง full) (3) ✅**reorder ▲▼** ปุ่มปักในแผง ⚙ (D6-style · persist · unit) (4) ✅**marker วงกลม** (min-height:0 กันยืด · 12×12) · test 143/143 · build ✅ · **รอบ 6 (P'Aim screenshot · 3 จุด):** (1) ✅**popup ลากย้ายได้** (แผง ⚙ + selector · ลากหัว ⠿ · verify เลื่อน 120,60) (2+3) ✅**timeline=ทั้งเพลงเสมอ · เลือก=เขียว** (เดิมยุบเหลือท่อนเดียว → แกน `fullNotes` · จุดครบเสมอ · picked เขียว · playhead map `posFullIndex` เล่นเฉพาะเลือกแต่ dot ตรงตำแหน่งจริง · verify song84: เลือกรับ1 ที่โผล่ 2 จุด→เขียว 2 · dot 25.74% แกนทั้งเพลง · active=รับ1) · test 143/143 · **รอบ 7:** **A** รีวิว "หน้าแก้ไข" = ส่งผิดสาย (edit-dev · ไม่แตะ) · **B** = เดียวกับรอบ 6 (done) + dropdown drag ยัง (panel+selector ลากได้พอ) · **C ✅ fit-content = core ทุกโหมด** (`.sd-dock:not(.sd-m)` · verify แผ่นเพลง 300<เดิม700 · แก้ไข 400 · ฝึกร้อง 488 · edit keys ใช้ได้ 31-40px desktop / mobile หดพอดี) · **D ✅ font bug แผ่นเพลง** (Studio.vue ครอบ `.sheet-read-scale` readingFontScale · verify Aa 180%→16→28.8px · print reset 1rem กัน A4) · test 143/143 · build ✅ | debug pl2 r1 |
