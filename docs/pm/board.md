...
| **B072 MP3 download** `mp3-download` | worktree `../pleng-mp3` (ยังไม่ push) | ✅ เสร็จ — client-side MP3 (OfflineAudioContext + lamejs) · `midi.js` แยก `scheduleNote` (เสียง=ปุ่มฟัง) + `audioExport.js` ใหม่ + ปุ่มใน `DownloadTool.vue` · **verify จริง เพลง100: pitch 201/201=100% · MP3 2.54MB/166s · decode กลับเล่นได้ PASS ✅** · เทสต์ 274 ผ่าน (+6, notationLint quirk เดิม) · build ผ่าน · report `docs/reports/mp3-download.md` · **หมายเหตุ:** DownloadTool ยัง orphan บนฐาน (dock=phase หลัง) → bundle ยังไม่โต · รอ P'Aim ฟัง http://10.215.141.98:5372/verify-mp3.html |
