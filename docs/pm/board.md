...
- **🐞 บั๊กพี่เปาชุดใหม่ (survey ทั้งโฟลเดอร์ 13 ก.ค.):**
  - **issues1 triplet+tie** → tie Dev `local_6a3414a8` (`task_6ff2cbb9`) กำลังทำ (เลข3 ชัด + tie โยงเลข→เลข · เพลง "ของขวัญ")
  - **issues2 เอื้อน vs slur** (`6_5_`/`1'_7_` ลากติดแต่ไม่ใช่ slur · เพลง "ในโลกนี้") → **รวมเข้า tie Dev สายเดิม** (lane เดียวกัน กันชน NoteRow/SongSheet) · อาจต้อง define notation เอื้อน (คุย DA/model — รอ report)
  - **issues3 "ดูผลทั้งเพลง" ≠ แผ่นเพลง (บรรทัดตก)** → **Dev สายใหม่** `task_73864e85` · **⚠️ collision SongSheet กับ tie Dev → PM merge tie ก่อน ให้ issues3 rebase**
  - **issues4** = รูปเพลง "พระเยซูเข้าสู่ใจข้า" **ไม่มีข้อความ** → **รอ P'Aim/พี่เปา อธิบาย** (ไม่เดา)
  - kanban พี่เปา: issue1/2/3 → 🔵 กำลังทำ + ตารางอัปเดตแล้ว
- **🎨 เมนูสไตล์พระคำ (desktop+mobile · P'Aim 13 ก.ค. · ภาพ `Screenshot_20260713_164458_Chrome.jpg`)** — แฮมเบอร์เกอร์+ไอคอนซ้ายบน+ค้นหา+หมวด "เครื่องมือ" (Aa/ตั้งค่า/ดาวน์โหลด · = phrakham pk-navbar) → **จ่าย SA เขียน DS `docs/ds/menu-parity.md`** (สาย `local_cd5fddfc`) · brief `docs/pm/brief-menu-parity.md` · **Dev เมนูรอ hero/favicon merge ก่อน (ชน shell-bar) + รอ P'Aim review spec**
- **🐞 บั๊ก repeat (พี่เปา · text) — รอ P'Aim ยืนยันคำทวนก่อนจ่าย** · อาการ: กดจุดหนึ่งในโน้ต → เล่นจากจุดกดถึงจบเพลง → วนกลับมาที่จุดกด (ผิด) · ต้องการ: เริ่มที่จุดกด (ถูก) แต่ **วนตามท่อนที่กำหนดตอนเลือกท่อนเพลง** ไม่ใช่วนจุดกด→จบ · เกี่ยว B102 play-order/loop · ยังไม่ dispatch
