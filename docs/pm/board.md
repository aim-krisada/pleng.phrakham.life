...
| **B043 dev `wt-b043-dev`** | (committed) | ✅ **เฟส 1 เสร็จ — รอ PM ตรวจ+merge** · report `docs/reports/wt-b043-dev.md` · **Network URL http://10.215.141.98:5323/** · music dock ครบ: transport (progress+marker+⏮▶⏭🔁+scrub+แตะจุด) + ⚙ settings panel (ทุก control inline + 📌pin persist) + selector (Gmail list+All/None · C=ไม่จำ) สร้างเป็น **core ผ่าน D8 region:'top'** (StudioDock เพิ่ม top-region ใหม่ + `SingTransport.vue` core reusable · ไม่ฝังใน SongViewer) · ตัด `.section-bar`/การ์ดบนออก · download เข้า panel · **B038** scroll เล็ง `[data-syl]` · **B042** ▶/⏸ ต่อ+⏮ กลับต้น · play/pause icon-only · F=เงียบ (v1) · engine additive (`effectiveOrder`/`buildPlayNotes`/`playSong order` · ไม่แตะจังหวะ/ทำนอง) · **test 134/134 · build ✅ · พิสูจน์เล่นจริง** (posIndex/dot/ไฮไลต์พยางค์เดินจริง · ⏮⏭ ตรง marker boundary · scrub 50% · selection→order 273→133 · ⚙ inline+pin) · **ค้างให้ P'Aim มือถือ:** auto-scroll ตามพยางค์ (preview scroll แบบ programmatic ไม่ได้ในเครื่องนี้ · unit ยืนยัน target=`[data-syl]` + ไฮไลต์เดินตรงพยางค์) · **เฟส 2** (A2 แผ่นย่อ+verify print PDF) รอ merge ก่อน · **note:** transparency ยังที่ปุ่ม block dock (alpha=state StudioDock · custom เอื้อมไม่ถึง) — ย้ายเข้า panel = dock-core follow-up | debug pl2 r1 |
