...
- **B107 (dev) ระบบเสียงเครื่องดนตรีจริง (sampler+presets+arranger)** — **จ่าย dev แล้ว (spawn_task 12 ก.ค. · branch `b107-instrument-playback` · `task_e0b9df46`)** · P'Aim เคาะ (ผ่าน PM): เอาเต็มระบบแทนเสียง synth B104 · **ไม่แก้ gain แยก** (กลืนในนี้) · live Tone.Sampler เสียงอัดจริง (ยังทรานสโพสได้) + preset (Grand default · P'Aim ชอบ #2 เปียโน+#4 เต็มวง) + auto-arranger 3 ชั้น + MP3 ใช้ sampler เดียว · CC0/CC-BY เท่านั้น · **⚠️ dev เสนอแผนเป็นเฟส+ปิง PM ก่อนลุยหนัก (โดยเฉพาะ host ไฟล์เสียง MB + เวลาโหลดมือถือ)** · SSOT `docs/ds/chord-voicing-quality.md` · brief `brief-b107-instrument-playback.md` · **เสียง/รสนิยม = P'Aim↔SA ตรง (memory `feedback_paim_direct_sa_creative`)**
...
- **B101 คัดลอก/วางบรรทัด-ห้อง ยืดหยุ่น — 🟠 tester CONDITIONAL · แก้ B2 ก่อน deploy** (branch `b101-copy-paste-flexible` · code `2d32af4` · dev `local_d3a5e953`) · tester ผ่าน fence/test/build/desktop + wording ชัด + B098 ไม่พัง · **P'Aim เคาะ:** B1 (เนื้อไม่ตามไปตอนวางท่อนใหม่=melody-only) → **รับ melody-only** (โค้ดถูก · "เอาเนื้อไปด้วย"=follow-up) · B2 (มือถือ เมนู ⋯ หลุดขอบ ทางเข้า copy-line) → **แก้ก่อน deploy** (clamp `.sb-dropdown`) · ✅ **dev แก้ B2 เสร็จ (clamp เมนู · commit `41e4c98`) · จ่าย tester re-check** (`task_278e01d5`) → PASS → PM cherry-pick `2d32af4`+`41e4c98` → deploy · report `docs/reports/tester-b101-copy-paste-flexible.md`
...
- **🟢 B107 P1 เสร็จ → ขอ tester gate** (branch `b107-instrument-playback` · report `docs/reports/b107-instrument-playback.md`) · PM เคาะ gate แล้ว (host-agnostic+jsDelivr · Grand default+fallback) · **P1 = เปียโน Grand จริงแทน synth B104 + แก้คอร์ดดังไป (voice-leading+บัส gain)** · **loading UX (P'Aim เปลี่ยน 12ก.ค.):** รอโหลด sample พร้อม **progress bar** (แบบ MP3 download) แล้วค่อยเล่น · synth = fallback เฉพาะโหลด fail · กดพักระหว่างรอ=ยกเลิก · **dev self-verify เบราว์เซอร์จริง:** โหลด 30 ogg **3.23MB** (1 velocity layer ไม่ใช่ 17MB) จาก PD host · progress tick 30 ครั้ง 0→1 monotonic · fire ทำนอง/คอร์ด/ทรานสโพส ไม่ error · 0 console error · vitest 436 ผ่าน+build ผ่าน · smplr = lazy chunk · **tester ต้อง:** ล็อกอินทีมเปิดเพลงจริง (anonymous ติด GATE 0 เพลง) · ฟังด้วยหู (เปียโนถูก/เพราะ) · มือถือ (`http://10.152.249.98:5307/` progress ลื่น+cache รอบสอง+กดพักยกเลิก) · regression เล่น/หยุด/สลับโหมด/ทรานสโพส/MP3 · **อย่าตี "เสียงเปลี่ยน synth→เปียโน" เป็นบั๊ก = ฟีเจอร์** · **P2/P3 ยังไม่ทำ:** presets+arranger (P2) · MP3 เปียโนจริง (P3 · ตอนนี้ MP3=synth voice-led) · mirror ไฟล์เสียงก่อน production เต็ม
