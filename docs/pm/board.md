...
| **DA import `da-import`** | `f8efb42` | 🚦 **GATE 1 เข้าแล้ว (9 ก.ค. เย็น) · PM git-verified** — report `docs/reports/da-import.md` (บน branch) · source = **DOCX เนื้อไทย + PDF geometry (pdfplumber) ทำนอง/ออกเทฟ** ใช้คู่กัน (114/120 ออกเทฟใน Word = วงกลมลอยจับไม่ได้ → PDF อ่านแม่น) · `tools/parse_song.py` + `samples/{001,010}.json` ตรวจผ่าน resolveContent+beat-checker (0 mismatch) · ✅✅ **GATE 1 ผ่าน — P'Aim เคาะครบ 5 (9 ก.ค. เย็น):** (1) source ผสม ✅ (2) ออกเทฟ auto+flag ✅ (3) v2 รวมทำนองซ้ำ ✅ (4) **key = คีย์เสียงจริงคอร์ด** (เพลง10=B) ✅ (5) นำร่อง 5 เพลง (1/10/100+สุ่ม2) ✅ · ✅ **Step 2 เสร็จ · PM git-verified (commit `65a0375`)** — นำร่อง 5 เพลง (1/10/50/90/100) ผ่าน resolveContent+beat-checker (0 mismatch ทุกเพลง) · parser อัปเกรด (barline จริง→pickup ถูก · key=คีย์คอร์ด) · ไฟล์: `tools/backup-songs.sql` (snapshot กันหาย · IF NOT EXISTS ปลอดภัย) + `tools/samples/pilot-5.sql` (upsert by number) + per-song REVIEW flags ครบ · **PM ตรวจ SQL = ปลอดภัย** · 🚦 **GATE 2 รอ P'Aim:** (1) run backup-songs.sql (2) run pilot-5.sql (3) เปิดแอปดู โดยเฉพาะเพลง 100 → go=แปลงครบ 120 (Step 3) / no-go=จุดแก้ · ⚠️ **คำเตือน PM:** pilot ทับเพลง 1/10/50/90/100 บน **DB จริง = เว็บ live เห็นทันที** (seed ยังมีจุดเกลา) · backup กู้ได้ · แนะ run ตอน P'Aim เกลาได้เลย | debug pl2 r1 |
