...
- **B107 (dev) ระบบเสียงเครื่องดนตรีจริง — 🏗️ dev ลุย P1 (PM อนุมัติ gate แล้ว)** (branch `b107-instrument-playback` · session `local_6536bc34`) · **แผน 3 เฟส:** P1 sampler infra + Grand default + กลืน gain/voice-leading fix + fallback synth (→ship+tester) · P2 presets+arranger 3 ชั้น · P3 MP3 · **PM เคาะ (วิศวกรรม):** (1) host-agnostic baseUrl · P1 ชี้ jsDelivr วัดโหลดมือถือก่อน → **mirror มาโฮสต์เราเองก่อน production เต็ม** · ห้าม self-host ในรีโป (2) **P'Aim เคาะ (แก้จาก PM): รอโหลด sample ครบก่อนค่อยเล่น + โชว์ progress แบบ MP3 download** (`DownloadTool`/`audioExport` มี progress) · synth = fallback เฉพาะโหลด fail · **วัดจริง:** Grand ~2-3MB (จำกัด notesToLoad · ห้าม 17MB) · สตริง ~4.4MB · 3G ~60วิ (fallback ครอบ) · smplr ตัวเดียว ไม่พึ่ง Tone.js · SSOT `docs/ds/chord-voicing-quality.md` · **เสียง/รสนิยม = P'Aim↔SA ตรง**
