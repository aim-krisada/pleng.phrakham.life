...
- **🚪 drawer-parity (P'Aim 14 ก.ค. · เลือก ก):** drawer เพลงเปิดจาก**ขวา** ≠ พระคำ (top-right toolbar dropdown + nav กางจากซ้าย) + ขัด "ชิดซ้าย" → SA `task_60c6dd33` **verify พระคำ live + เสนอ fix → `docs/ds/menu-drawer-parity-fix.md` → PM+pk pm4 เคาะ → dev** (pre-existing ตั้งแต่ร22 · ไม่ใช่บั๊กใหม่) · **PM บทเรียน: อย่าเดา parity — เปิดดูจริง (feedback_verify_parity_live_computed)**
- **🎵 notation:** พี่เปา print #8/#12/เพลงเอื้อน เทียบหนังสือ (live) · เพี้ยน→แก้รอบหน้า
- **🎹 ROUND 2 audio:** `docs/reports/handoff-audio-round2.md` (tune Grand เลอเลิศ + เมนูเทคนิคละเอียด + Remix[ต้อง UX designer] + timecode)
...
- **✅ drawer-parity verify เสร็จ (SA · `task_60c6dd33`)** → `docs/ds/menu-drawer-parity-fix.md` · **verify live 412px ทั้ง 2 เว็บ (ไม่เดา):** พระคำ **ไม่มี drawer ด้านข้าง** — กด ☰ = เมนูกางลงเต็มกว้างใต้แถบหัว (Bootstrap collapse · `static` ดันเนื้อหาลง · ชิดซ้าย · ไม่มีฉากดำ) · เพลง = แผงลอยยึดขวาเลื่อนจากขวา สูงเต็มจอ + ฉากดำ → ต่าง 6 จุดโครงสร้าง + ขัด "เริ่มจากซ้าย" · **SA แนะ Option A** (เปลี่ยนเพลงกางลงเต็มกว้างแบบพระคำ = ตรงที่ P'Aim ว่า "nav กางจากซ้าย" · scope `ShellBar.vue`+`styles.css:355-372/429/437` · เสี่ยงกลาง) · fallback Option B (ยึดซ้าย ~5 บรรทัด) · **🚩 สเปกร่วม `menu-drawer-spec.md` บรรทัด 9 เขียน "baseline = phrakham drawer" = ผิด (พระคำไม่มี drawer) → ต้องแก้** · **รอ pk pm4 + P'Aim เคาะทิศก่อนจ่าย dev**
