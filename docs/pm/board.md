...
- **B104 (SA วิเคราะห์) MIDI ใส่เสียงคอร์ดมือซ้าย — ✅ SA เสร็จ · รอ P'Aim เคาะ 5 ข้อ** (branch `b104-sa-midi-chords` · session `local_b852f9a3`) · **สรุป: ทำได้ งานเล็ก-กลาง ไม่ต้องเพิ่ม lib** (มี segment.chord + synth scheduleNote อยู่แล้ว → เพิ่ม suffix→intervals ใน chords.js + buildChordVoice ใน midi.js · MP3 sync ฟรี) · spec `docs/ds/midi-chord-accompaniment.md` (AC 9 ข้อ) · **5 คำถาม:** (1)default เปิด/ปิด [SA:ปิด] (2)v1 block ก่อน arpeggio ทีหลัง (3)voicing ราก+3+5 vs +เบสต่ำ (4)ขอบเขต ฟังท่อน+ทั้งเพลง+MP3 vs แค่ในเว็บ (5)ลิงก์เว็บ hymnal อ้างอิง · ▶ P'Aim เคาะ → PM จ่าย dev
...
- **B099 เส้นโค้ง slur/ไท วาดขาด (โบว์→arc เดียว)** — ✅ **DEV เสร็จ · จ่าย tester gate แล้ว** (branch `b099-slur-curve` @ `2af393d` · dev `local_ad7c95b6` · tester `task_ef3a7056`) · **ต้นเหตุจริง = ไท `~` ในห้องเดียว** (2 ครึ่งออกแบบให้คร่อมเส้นแบ่งห้อง · ไม่มีเส้นคั่นเลยเหลื่อมเป็นโบว์) · แก้ที่ overlay `SongSheet.measureTies()` (B069) ให้วาด arc เดียวครอบไทในห้องเดียวด้วย · renderer ตัวเดียว = แก้ไข+แผ่นเพลง+พิมพ์ · ไม่แตะ NoteRow · 382 test + build · verify browser (โบว์→crescent เดียว · slur ไม่ regress) · report `docs/reports/b099-slur-curve.md` · ▶ รอ tester PASS → PM deploy · **ยังไม่ deploy**
