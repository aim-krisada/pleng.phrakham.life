...
- **🔴 บั๊กค้าง: karaoke highlight ไม่ advance** (เพลงเล่นได้ เสียง+เวลาเดิน แต่คำไม่เลื่อนตาม) · **ไม่ใช่ regression + ไม่ใช่เรื่อง load** (SA เดาผิด · PM แก้ความเข้าใจแล้ว) · SA (`local_8d2bfd37`) เจาะ playingSyl←onNote ต่อ · รายละเอียด §🎹 ROUND 2
...
- **🎹 ROUND 2 audio (P'Aim 14 ก.ค. · ทิศ: โหลด sample ครบก่อน → ออกแบบจากของจริง · ไม่ band-aid):**
  ...
  - **🔴 GATE จับได้ก่อน push (14 ก.ค.): karaoke highlight ไม่ขึ้น (real device incognito :5420)** — P'Aim/พี่เปาทดสอบ **เพลง 1 พระเจ้าเป็นความรัก** (`f266baf9`) = **มีโน้ต 47 ตัว + arrangement ครบ (ไม่ใช่เนื้อล้วน!)** → highlight พังบนเพลงมีโน้ตด้วย = กว้างกว่าที่คิด · **สงสัย regression จาก round 2 (songToNotes/midi.js)** · ✅ round 2 merge base แล้ว (626 tests) แต่ **HOLD push จนแก้** · ส่ง SA ตรวจด่วนแล้ว (`local_8d2bfd37`) · **P'Aim ตอบ: เพลงเล่นได้ (เสียง+เวลาเดิน) แต่ highlight ไม่ขยับ = บั๊กที่ตัวไฮไลต์ (li/si index ไม่เดินตาม playback) ไม่ใช่ playback** → ส่ง SA เจาะ onNote/tick→highlight sync · **⚠️ SA เดาผิดรอบแรก (ว่าเป็น Grand 12MB โหลดไม่ขึ้น/ไม่เล่น) — P'Aim ยืนยันเพลงเล่น = ตัดทฤษฎีโหลด · PM แก้ความเข้าใจ SA แล้ว: อาการ = เล่นได้แต่ highlight ค้าง/ไม่ advance (SA เห็น '.syl-playing 1 ตัว' = อาการค้าง ไม่ใช่ทำงาน)** · SA เจาะใหม่ทำไม playingSyl ไม่เลื่อนตาม onNote(ยิง 35 ครั้งพร้อม syk) · **HOLD deploy · P'Aim จะแก้ต่อ PM session ใหม่**
...
## 📥 inbox → PM
- **🎻 ไวโอลินเดี่ยว expressive — เดโมพร้อมให้ P'Aim ฟัง (SA · branch `claude/charming-cori-25514b`)** → `docs/spikes/violin-solo-demo.html` · report `docs/reports/violin-solo.md` · **พบ: sample+registry+bowed module+ensemble ของไวโอลินมีในฐานแล้ว** (VSCO-2 CC0 self-host) · ที่ขาด = (ก) ปุ่มยัง `disabled` (`soundOptions.js`) (ข) solo `sampler.fire` ยิงทื่อ ไม่มี bow-attack/สเวลล์/legato/vibrato → **ต้องมี bowed playback path แยก** (buffer-source + envelope ต่อโน้ต · smplr ทำ envelope ต่อโน้ตไม่ได้) · เดโมทำ path นั้นรอบเสียงจริง + toggle ทีละลูกเล่น · **verify: live 0 error โหลดไฟล์จริง · offline peak 0.226>0.01 (ทุกลูกเล่นออกเสียง · ไม่ติด smplr lookahead)** · **ยังไม่แตะ `src/` · ยังไม่ merge — creative iterate: P'Aim ฟัง+เคาะค่าก่อน → แล้วค่อยจ่าย dev wire เข้าแอป → PM gate** · Network URL มือถือ = `http://192.168.1.124:5344/docs/spikes/violin-solo-demo.html`
