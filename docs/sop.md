...
## 4.1 · 📘 กฎถาวร: แก้ซอฟต์แวร์ = ต้องอัปคู่มือเสมอ (P'Aim 20 ก.ค.)
**ทุกครั้งที่มีการปรับปรุงซอฟต์แวร์ (ฟีเจอร์ใหม่ · เปลี่ยนพฤติกรรมที่ผู้ใช้เห็น · ปุ่ม/ตำแหน่ง/ค่าเริ่มต้นเปลี่ยน) → ต้องอัป `src/views/Guide.vue` (คู่มือผู้ใช้) ในงานเดียวกันเสมอ**
- **อยู่ใน DoD:** งานยังไม่ถือว่าเสร็จถ้าคู่มือยังไม่อัป · **PM ไม่ gate ผ่าน** ถ้า user-visible change แล้วไม่มีคู่มือ
- **อยู่ใน brief:** ทุก brief ที่เปลี่ยนสิ่งที่ผู้ใช้เห็น ต้องระบุ "อัปคู่มือด้วย" เป็น deliverable · **tester ตรวจว่าคู่มือตรงกับของจริง**
- **ภาษาคู่มือ:** ภาษาคนล้วน ระดับอาสาสมัครคริสตจักร (พี่เปาอ่านรู้เรื่อง) · ตามโครง/โทนเดิม ไม่ redesign
- **ยกเว้น:** งานที่ผู้ใช้ไม่เห็นเลย (refactor ภายใน · เทสต์ · CI · perf ที่ไม่เปลี่ยนหน้าตา) — ระบุในรายงานว่าทำไมไม่ต้องอัป
- เหตุผล: คู่มือที่ล้าหลัง = ผู้ใช้ไม่รู้ว่ามีฟีเจอร์ + สอนผิด · เจอจริง 20 ก.ค. (branch `guide-update` ค้าง 116 commits ไม่ถูก merge) · [[pleng-guide-always-updated]]

## 5 · Conventions
- **brief:** ทุก brief SA/dev ลิงก์ `ui-standards.md` + DoD self-verify · **ระบุไฟล์ที่จะแตะ** (collision) · **+ ระบุ "อัปคู่มือ" ถ้า user-visible (§4.1)**
...
- **⭐ ไม่แน่ใจ = ห้ามเดา (P'Aim 18 ก.ค.: "ถามผมก่อนลงมือ กันเปลืองโทเคน/เวลา"):** แยก 2 แบบ — (ก) **ไม่แน่ใจ "P'Aim อยากได้อะไร/ขอบเขต/จะเปลี่ยนพฤติกรรมของเดิมไหม" → PM หยุด ถาม P'Aim ก่อน build** (ไม่ให้ทีมลงมือบนสมมติฐาน) · (ข) **ไม่แน่ใจเทคนิค/มาตรฐาน → resolve เอง** (อ่านโค้ด/SA/dev/spec/standard · ไม่กวน P'Aim · = anti-pattern §6 #6 "ห้ามถามอะไรถูก") · **การเปลี่ยน UI/พฤติกรรมที่มีอยู่เดิม (พี่เปาใช้อยู่) = ต้อง confirm P'Aim ก่อน ไม่ใช่ declutter บนสมมติฐาน**
...
10. **สเปกบอก "ใช้/ต่อยอดของเดิม" = ใช้ตามเดิมเป๊ะ · ห้ามเปลี่ยนหน้าตา/เพิ่มปุ่มใหม่/redesign (ทำเกิน · gold-plating)** — บทเรียน 18 ก.ค.: R4 บอก "ต่อยอด 📌 pin เดิม" แต่ dev ไปเพิ่ม ✕/＋ ใหม่แทนพินแดงเดิม → P'Aim: *"จะเปลี่ยนทำไม ทำเกินทำไม"* · **PM เขียนสเปกให้ชัด "ใช้ X เดิม ไม่เปลี่ยนหน้าตา ห้ามเพิ่มของใหม่" + gate จับว่า deliverable ไป*เปลี่ยน/เพิ่ม*ของเดิมที่ไม่ควรแตะไหม** · ทำเท่าโจทย์ ไม่ทำเกิน · `feedback_refine_not_redesign`
11. **gate ที่ "ผู้ใช้ทำได้จริง (functional)" ไม่ใช่ "กลไก/ตัวเลขผ่าน"** — 18 ก.ค.: tester ยิง synthetic `dispatchEvent` ข้าม hit-test = "ลากผ่าน" แต่คนลากจริงไม่ได้ · ปุ่ม ≥24px "ผ่าน WCAG" แต่จิ๋วกดไม่ได้ · **สเปกต้องมี §FUNCTIONAL ACCEPTANCE (ผู้ใช้ทำอะไรได้บ้าง) ไม่ใช่แค่กลไก · tester ทดสอบ real interaction (ลาก/กดจริง) ห้าม fake PASS · PM ใช้วิจารณญาณดูตาเปล่า** (เห็นปุ่มจิ๋ว = รู้เลยใช้ไม่ได้ ไม่ต้องถาม P'Aim "ใช้ไม่ได้ตรงไหน") · `feedback_verify_hover_on_real_browser` · `pleng-dock-resize-usability`
