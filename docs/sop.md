...
| **UX/UI (ถาวร)** | *"ผู้ใช้เห็น/ทำอะไร · ทั้งเว็บเป็นอันเดียวกันไหม"* — flow · IA · visual consistency · US/DS+mockup อิง `ui-standards` + **[`ux-platform-patterns.md`](ux-platform-patterns.md) (touch≠pointer + เช็คของเดิม · ผูกพัน)** · **เจ้าของความ consistent ทั้งเว็บ** (docs) · `-uxui` |
...
**โมเดลบังคับ (แทน `SA: US+DS+mockup`):**
1. **ออกแบบบน component จริง** (`DockKey.vue`/`EditorMode.vue` ฯลฯ) เป็น **working prototype บน branch จริง (dev-ready)** ที่รันในแอปจริง — **สิ่งที่ P'Aim เห็น = สิ่งที่ขึ้นจริง ไม่มีดริฟต์** · ⛔ ไม่มี HTML mockup ทิ้งอีก (UX ขับ design · dev implement บนโค้ดจริงตั้งแต่ step 1 · คู่กัน 1 branch)
2. **สเปกครบชิ้นเดียว** — ทุกปุ่ม/ตั้งค่า/state/breakpoint/a11y/device-matrix ในที่เดียว · dev ไม่เดา · P'Aim ไม่พูดซ้ำ
3. **ส่งทีเดียวครบ + tester gate ก่อน** → P'Aim เห็นของสมบูรณ์ใช้ต่อได้จริงชิ้นเดียว · ปรับ = แก้บน branch จริงอันเดิม (ไม่ re-implement · ไม่ iterate ทีละบั๊ก)
_(งาน docs/สเปก/วิเคราะห์ล้วนที่ไม่ลงจอ component = ยังเป็น docs ได้ · exploratory mockup เร็ว ๆ ใช้ได้เฉพาะ "คุยทิศภายในทีม" ไม่ใช่ deliverable ถึง P'Aim)_

```
[งาน component] idea → complete-spec + working prototype บน component จริง (UX ขับ + dev implement · 1 branch)
