...
## 5.5 · ⛔⛔ verify ที่ "ของจริง" ไม่ใช่ที่ "สัญญาณ" (บทเรียนแพงสุด 20 ก.ค. · P'Aim หยุดงานทั้งหมด)

**§6 ข้อ 5 เขียนกฎนี้ไว้ตั้งแต่ 17 ก.ค. แล้ว — และ PM ก็ยังทำผิดซ้ำ 3 ครั้งในวันเดียว**
→ ปัญหาไม่ใช่ "ขาดกฎ" แต่คือกฎที่เป็นร้อยแก้ว **อ่านแล้วเห็นด้วย แต่ตอนทำงานจริงไม่มีอะไรบังคับ**
§5.5 จึงไม่ใช่กฎเพิ่ม แต่คือ **การเปลี่ยนกฎข้อนั้นให้เป็นแบบฟอร์มที่กรอกไม่ครบไม่ได้**

### กติกาบังคับ — ทุกรายงาน ทุก session ทุกใบ ต้องมี 3 ช่อง ห้ามขาด
1. **แก้อะไร** — diff/commit จริง
2. **พิสูจน์ยังไง** — คำสั่งที่ copy ไปรันซ้ำได้ + **output ดิบที่แปะมา** (ไม่ใช่ "ตรวจแล้วถูก")
3. **อะไรที่ยังไม่ได้พิสูจน์** — **ช่องนี้ว่างไม่ได้ · ว่าง = งานยังไม่เสร็จ · PM ไม่ gate**

### คำที่ห้ามใช้
- ⛔ **ห้ามเขียน "ผ่าน / เสร็จ / แก้แล้ว / ใช้ได้แล้ว" ถ้าไม่ได้เปิดของจริงดู** — เขียนได้สูงสุดแค่
  **`self-verified (layer=<ชั้นที่วัด>)`** · คำว่า **`cross-verified` สงวนไว้ให้ผู้ตรวจอิสระเท่านั้น**
- ⛔ **ระบุชั้นที่วัดในตัวประโยคเสมอ** — "ไม่มี X ในซอร์ส" จริงและไร้ประโยชน์ · "ไม่มี X บนหน้าเว็บ" คือคนละเคลม
  ชั้น: `source` → `build-output` → `live-page` → `printed-pdf` → **`human-eye`**
- ⛔ **`human-eye` ไม่มี AI เซ็นได้** (สวยไหม/เพราะไหม/เส้นขาดไหม/กดง่ายไหม) — เตรียม URL ให้คนดูเท่านั้น

### สัญญาณที่ถูกจับได้แล้วว่าโกหก (ของจริง 20 ก.ค. ทั้ง pleng + phrakham)
**ไม่มีแถวไหนจับได้ด้วยการอ่านโค้ด/config/diff — จับได้เพราะเปิดของจริงดูเท่านั้น**

| สัญญาณที่บอกว่าสำเร็จ | ของจริงที่บอกความจริง |
|---|---|
| นับเส้นในไฟล์ได้เลขถูกทุกตัว → สรุปเองว่า "คานหาย 92%" | รันฟังก์ชันที่ผลิตมันออกมาหาค่าคาดหวังก่อน (ตรงกันเป๊ะ ≤0.1pt = ไม่มีบั๊ก · เกือบสั่งรื้อ 3-5 วัน) |
| agent บอก "`resize_window` พัง ทดสอบมือถือไม่ได้" | ลองเอง → ใช้ได้ปกติ (agent ลืมส่ง `tabId`) · **ห้าม relay "ทำไม่ได้" โดยไม่ลองเอง** |
| dev + tester ยืนยันตรงกันว่า "คานหายหลัง 4 วิ" | ทั้งคู่วัดจาก agent browser `visibilityState:hidden` · rAF ไม่ยิง = **env พังอันเดียวกัน** |
| exit code 0 | ไฟล์ผลลัพธ์ที่ render ออกมา (เครื่องมือที่พังเงียบ อันตรายกว่าพังดังๆ) |
| mtime ของไฟล์ใหม่ | grep เนื้อหาใหม่ใน build (mtime เด้งหมดแม้เนื้อไม่เปลี่ยน) |
| gate ผ่าน | เอา gate ไปรันกับ tree ที่จงใจทำพัง — **gate ที่ผ่านทั้งที่ของหาย แย่กว่าไม่มี gate เพราะคนเชื่อมัน** |
| ลบโค้ดฟีเจอร์แล้ว | probe ไฟล์ผลลัพธ์ (ของที่ลบแล้วรอดมาบ่อยกว่าที่คิด) · แบนเนอร์ในซอร์ส → grep ใน bundle ที่ minify |

### ผู้ตรวจอิสระ (P'Aim 20 ก.ค. — "ตอนนี้ผมไม่เชื่อคุณแล้ว")
Antigravity เข้ามาเป็น **pair/ผู้หักล้าง** ของ Claude Code · ช่องทางสื่อสาร = **ไฟล์ในกิต ไม่ใช่แชท**
· โปรโตคอล + เทมเพลต: **`docs/pair/README.md`** (อ่านเองจบ ส่งให้ AI ตัวไหนก็เข้าใจ)
· 1 งาน = 1 ไฟล์ `docs/pair/<task>.md` · **Claude เขียนได้เฉพาะ `## CLAIM` · Antigravity เฉพาะ `## VERDICT` · คนเฉพาะ `## HUMAN`**
· **ขัดกัน = รันซ้ำคนละ 1 ครั้ง แล้วหยุด ส่ง P'Aim พร้อม output 2 ฝั่ง — ห้ามหาข้อสรุปกันเอง**
· เหตุผลที่ต้องเป็นแบบนี้: B118 มี 3 ฝ่ายให้ 3 คำตอบ เพราะทุกฝ่าย**ส่งข้อสรุปให้กันอ่าน ไม่ได้ส่งวิธีรันซ้ำให้กันรัน** → ความมั่นใจเพิ่มทุกชั้น หลักฐานเท่าเดิม

## 6 · ⛔ PM anti-patterns (บทเรียน 2026-07-17 · PM session หน้าอ่านก่อนเริ่ม)
> **ข้อ 5 ถูกยกระดับเป็น §5.5 แล้ว** (เขียนไว้เฉยๆ ไม่พอ — ต้องมีแบบฟอร์มบังคับ)
