...
## ปรับรอบ 4 (P'Aim ลอง 5323 · 5 จุด)
1. **grip ออกนอกกรอบ timeline** — ย้ายเส้นกรอบไปที่ตัว timeline (`.mp-framed`) · grip เป็น sibling อยู่ซ้ายนอกกรอบ · verify: `.mp-framed .mp-grip` = ไม่มี · grip เป็นลูกตรงของ `.mp-r1`
2. **✅ drag กลับมาแล้ว (bug fix)** — StudioDock ส่ง handler `grip-down/move/up` (= combinedDown/Move/Up เดิม B037) ให้ top-region control · grip เดสก์ท็อป bind pointer → **ลาก = ย้ายทั้ง dock · แตะ = หุบ** · มือถือ grip = แตะหุบ (ไม่ลาก) · **พิสูจน์เบราว์เซอร์: ลาก grip → dock ขยับ (140,90) เป๊ะ · แตะ grip → collapse+FAB โผล่** (เพิ่ม resize listener ให้ mobile detection sync)
3. **key = dropdown** — เปลี่ยนจาก stepper ◀E▶ → menu (badge+dropdown เหมือนความเร็ว/แสดงผล) · verify: `.mp-pins[key] .mp-pbtn` menu · badge = คีย์ปัจจุบัน
4. **ปุ่ม content-size** — ปุ่มใน music dock เป็น content-width อยู่แล้ว (badge buttons hug · transport = square tap target 40/50px คง WCAG) · dropdown fit-content (#5) คลุมเคสที่เห็น · **ถ้ามีปุ่มเจาะจงยัง excess ขอ P'Aim ชี้ (ภาพที่ส่ง = grip/key/dropdown ไม่ใช่ปุ่ม)**
5. **dropdown fit-content (core)** — `.mp-dd` (sing) + `.sd-pop-menu` (StudioDock ทุกโหมด) = `width:max-content` + row `nowrap` → เมนูความเร็วไม่ตัดบรรทัดมั่ว · verify: tempo dropdown ทุกแถว single-line (37px) · กว้าง 223px fit · อยู่ในจอ (clamp คุม)
- test 142/142 · build ✅ · error console = HMR hot-swap เก่า (timestamp เดิมทุก reload · `@vite/client`) ไม่ใช่ runtime

## ปรับรอบ 5 (P'Aim ลอง 5323 เพลงมีท่อน · 4 จุด)
1. **✅ collapse-in-place (grip ไม่กระโดด)** — StudioDock `collapse()` วาง FAB ที่ grip ที่กด (เก็บ `lastGripEl` ตอน combinedDown · ใช้แทน handleEl ที่ null ในโหมด music dock) · **พิสูจน์เบราว์เซอร์: จุดกลาง grip [322,640] = จุดกลาง FAB [322,640] เป๊ะ** (ไม่กระโดด)
2. **✅ dock กว้างเท่าปุ่มจริง (fit content)** — เพิ่มคลาส `.sd-fit` (เฉพาะ music dock ล้วน · เดสก์ท็อป) → `width:fit-content` + `min-width:340` + `max 700` · progress bar กว้างตามแถวปุ่ม (ปุ่มเยอะ=ยาว น้อย=สั้น) · verify: dock กว้าง 488 (ไม่เต็ม 700) · มือถือคง full-width
3. **✅ reorder ปุ่มปัก** — เพิ่ม ▲▼ ในแผง ⚙ (เฉพาะปุ่มที่ปัก · pin order = bar order · persist) แบบ D6 · unit test ยืนยัน (key ▼ → tempo,key,display + persist)
4. **✅ marker วงกลม** — เพิ่ม `min-height:0`+`box-sizing` กัน global button min-height ยืดเป็นวงรี · verify: 12×12 เท่ากัน (วงกลมจริง)
- test 143/143 · build ✅ · verify เบราว์เซอร์ #1/#2/#4 ครบ · #3 unit

## ปรับรอบ 6 (P'Aim ลอง 5323 · screenshot 3 จุด)
1. **✅ popup ลากย้ายได้** — แผง ⚙ + รายการเลือกท่อน ลากที่หัว (⠿) ย้ายตำแหน่งได้ (offset reactive · clamp ตอนเปิด + ลากทับได้) · เดสก์ท็อป · **verify: ลากหัวแผง → เลื่อน (120,60) เป๊ะ**
2+3. **✅ timeline = ทั้งเพลงเสมอ · ที่เลือก = เขียว** — เดิมเลือกท่อนแล้ว timeline ยุบเหลือท่อนเดียว → เปลี่ยนแกน timeline เป็น **ทั้งเพลง** (`fullNotes`) · จุดท่อน**ทุกจุดโชว์เสมอ** · ที่เลือก = สีเขียว (picked) · playhead map ขึ้นแกนทั้งเพลง (`posFullIndex`) เล่นเฉพาะที่เลือกแต่ dot วิ่งตรงตำแหน่งจริงของท่อนในเพลง · scrub/⏮⏭ snap เข้า selection
   - **verify song 84:** เลือก 1 ท่อน → marker ครบ 5 ยังอยู่ · ที่เลือกเขียว · **เลือก "รับ 1" (โผล่ 2 จุด) → เขียว 2 จุด** ✓ · เล่น selection → dot 25.74% (ตำแหน่งจริงบนแกนทั้งเพลง · ไม่ใช่ 0-100% ของ selection) · active marker = รับ 1 · syllable "พระ" ถูก
- test 143/143 · build ✅

## ปรับรอบ 7 (P'Aim ลอง 5323 · fit-content core + font bug + popup drag)
- **A. ข้อความรีวิว "หน้าแก้ไข" = P'Aim ส่งผิดสาย (งาน edit-dev)** — ไม่แตะ EditorMode · B043 = หน้าฝึกร้องเท่านั้น
- **B (3 จุด) = เดียวกับรอบ 6 ที่ทำแล้ว** (popup drag + timeline ทั้งเพลง + เลือก=เขียว) · เพิ่ม: dropdown draggable — **panel + selector ลากได้แล้ว** (ตัวที่บังเนื้อ) · dropdown เมนู = clamp + ปิดเองเมื่อเลือก (transient) ยังไม่ลาก · ถ้า P'Aim ยืนยันให้ dropdown ลากด้วยจะเพิ่ม
- **C. ✅ fit-content = core StudioDock ทุกโหมด** — ย้าย `.sd-fit` → `.sd-dock:not(.sd-m)` (ทุกโหมด · เดสก์ท็อป) · **verify: แผ่นเพลง 300 (เดิม 700) · แก้ไข 400 · ฝึกร้อง 488** · แป้นโน้ต edit ยังใช้ได้ (desktop min-width 30 → 31-40px · mobile min-width:0 หดพอดีจอ) · overflow (D3) ยังทำงานเมื่อเกิน 700
- **D. ✅ font bug หน้าแผ่นเพลง** — Aa ไม่มีผลเพราะ `Studio.vue` render SongSheet ตรงๆ ไม่ครอบ scale → ครอบ `.sheet-read-scale :style fontSize:readingFontScale` · **verify: Aa 180% → แผ่นเพลง 16px→28.8px** · **print reset เป็น 1rem** (กัน A4 พัง · verify PDF จริงเป็นงาน P'Aim)
- test 143/143 · build ✅ · verify 3 โหมด (dock พอดี · edit keys ใช้ได้ · sheet font scale)
