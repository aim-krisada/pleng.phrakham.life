...
**Scope:** รอบ 1 = B048 · B050 · B051 (merged `316d02d`) · **รอบ 2 = c/B049 (ตัด textarea)**
**แตะแค่ `EditorMode.vue` ทั้ง 2 รอบ** — ไม่แตะ SongViewer / StudioDock / SingTransport / ShellBar / store.js / FontTool / **SongSheet** ✅

---

## 🆕 ROUND 2 — c/B049 (E4): ตัดช่องพิมพ์ "ลำดับเพลง" ก้อนซ้ำ

**commit `e8b5d1d`** (ฐาน reset ไป `studio-shell-redesign` ล่าสุดหลังรอบ 1 merged แล้ว)

**SSOT = `docs/us/ps3-editor.md` E4** (brief ระบุให้ยึด): *"ตัด textarea ในหน้าลำดับเพลง · เหลือ รายพยางค์ + พาเนลพิมพ์เนื้อทั้งข้อ · 2 ที่ sync กัน"*

**ทำ:**
1. ❌ **ตัด `<textarea>` เนื้อร้องรายข้อ** ในส่วน "ลำดับเพลง" (ตัวที่ prototype ไม่มี · แก้เนื้อซ้ำที่)
2. ✅ **คงกล่องรายพยางค์ใต้โน้ต** (ไม่แตะ)
3. ✅ **คงพาเนล "📝 แก้เนื้อแบบย่อหน้า (ข้อที่เลือก)" ยุบ/กางได้** (มีอยู่แล้ว · ไม่แตะ logic)
4. ➕ เพิ่มปุ่ม **✎ "เลือกข้อนี้เพื่อพิมพ์เนื้อ"** ในแต่ละแถว (เพราะ textarea หายไป ต้องมีทางเลือกข้อมาพิมพ์)
5. อัปเดตหัวข้อ + คำใบ้ที่ยังชี้ให้พิมพ์เนื้อที่ "ลำดับเพลง"

**⚠️ จุดที่ขอ PM/P'Aim ยืนยัน (ผมตัดสินตาม E4 · ต่างจากคำว่า "ตัดก้อนใหญ่" นิดหน่อย):**
- ผม **ตัดแค่ textarea เนื้อร้อง** ตาม E4 · **เก็บแถวจัดลำดับไว้** (dropdown ท่อน · ชื่อข้อ · คีย์ · ▲▼ · ลบ) เพราะถ้าตัดทั้งก้อน = **สร้างเพลงหลายข้อ/หลายท่อนไม่ได้** (เลือกท่อนต่อข้อ · ตั้งคีย์ต่อข้อ · จัดลำดับ หายหมด) · prototype แยกงานนี้ไป view "ลำดับการร้อง (จัดเรียง)" ที่**ยังไม่ได้ build** → เก็บไว้ที่นี่เป็นสะพานก่อน
- ถ้า P'Aim อยากให้หน้าตาเป๊ะ prototype (ไม่มีส่วนนี้เลย) → ต้องให้ SA ออกแบบ view "จัดเรียง" แยกก่อน แล้วค่อยย้าย · บอกได้ผมทำรอบต่อไป

**พิสูจน์เบราว์เซอร์ (เพลงจริง/สด · server 5325):**
- `.arr-row textarea` = **0** (textarea เนื้อร้องหายจากลำดับเพลง ✅) · แถวจัดลำดับ+ปุ่มครบ (ท่อน/คีย์/▲▼/✎/ลบ)
- กด ✎ → lens ทำงาน (`lensChoice=0, lensActive=true`) → พาเนล "📝 แก้เนื้อแบบย่อหน้า" โผล่ ✅
- ใส่โน้ต "1 2 3" → กล่องรายพยางค์ 3 ช่องโผล่ใต้โน้ต · พิมพ์ "รัก พระ องค์" ในพาเนลย่อหน้า → **กล่องรายพยางค์ sync เป็น ["รัก","พระ","องค์"]** ✅ (2 ที่ = ข้อเดียวกัน ตรง E4 AC)
- **test 146/146 · build ผ่าน · ไม่มี console error**
