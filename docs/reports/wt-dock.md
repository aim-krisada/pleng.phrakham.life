...
## รอบ 3 — blur ผูกกับความโปร่ง (จิ๋ว 1 จุด)
- **ปัญหา:** `backdrop-filter: blur(6px)` เดิมเป็นค่าคงที่ → ต่อให้เลื่อนโปร่งสุด (40%) ก็ยังเบลอ บังเนื้อ/โน้ต
- **แก้:** ผูก blur กับ `--dock-alpha` — `blur(max(0px, calc((var(--dock-alpha) - 0.4) / 0.6 * 6px)))` → โปร่งสุด (0.40) = blur 0 (เห็นเนื้อชัด) · ทึบสุด (1.0) = blur 6px (frost เต็ม) · เพิ่ม `-webkit-` prefix ด้วย
- **พิสูจน์สด:** เลื่อนสไลเดอร์ 2 สุด → 40%→`blur(0px)` · 70%→`blur(3px)` · 100%→`blur(6px)` ✅ · test 113/113 · build ผ่าน
