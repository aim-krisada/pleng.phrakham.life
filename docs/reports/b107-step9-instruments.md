...
## 🔺 Ensemble round 2 — lead-driven + real sections + role-prominence (P'Aim 13 ก.ค.)

- **🔴 เต็มวง ไม่ตามเครื่องที่เลือก (แก้แล้ว):** เครื่องที่เลือก = **ตัวนำวง (เล่นทำนอง)**. grand → **เปียโนนำ** (grand ทำนอง+arp · cello เบส · violin wash) · nylon → **กีตาร์นำ** (nylon fingerpick ทำนอง · grand arp · cello เบส · violin wash · โหลด nylon เพิ่มตอน guitar-lead · §6b.1 ensemble-guitar). wire จาก leadInstrument (SongViewer+EditorMode) · violin-lead structured.
- **Section dynamics จาก section จริง (§6b.2):** `sectionBeatRanges()` อ่าน `{type:'section'}` จากแผ่น → beat ranges (ไม่ใช่ %บีต) · **master gain ramp verse→chorus ที่ขอบท่อน** · **default = chorus (เต็ม) ถ้าไม่เจอ refrain ชัด** → เต็มวงไม่ค้างจืด (P'Aim).
- **Role-prominence (แก้จืด · §6b.2):** ต่อ chord span — **หรี่ comp+bass เมื่อทำนองวิ่งถี่** (เปิดที่ให้พระเอก) · **เติม+ดังขึ้นเมื่อทำนองลากยาว** (เติมช่องว่าง). ค่า 4 ตัว (sectionGain 0.72/1.0 · duck 0.72 · denseThresh 0.9 · fillBoost 1.15) = **named constant · SA↔P'Aim จูนด้วยหูตอนฟัง final**.
- **ท่อน icon → `list-music`** (P'Aim เลือก · ไม่ใช่ table-of-contents).

**Verify (LIVE per-role peak-hold):** **guitar-lead → nylon นำ** (0.451 > grand 0.195 > cello 0.151 > violin 0.09) · **piano-lead → grand นำ** (0.405) · ทั้งคู่ audible ครบ + ไม่ clip. vitest 517 เขียว · build ผ่าน. **พักไว้ (ยังไม่ทำ · PM สั่ง):** rubato · crossfade pad · guitar fret-voicing.

## 🎼 Ensemble Option 1 (call-response · ไม่มี pad) — P'Aim เคาะขึ้น launch (13 ก.ค.)

P'Aim ฟัง Option 1 แล้ว "ดีพอสำหรับ 1st release" → **แทนสูตรเต็มวงเดิมที่มี pad** (port `ensemble-callresp-demo.html` · §6b.2 · มี `window.__peaks()`).

- **ตัด pad สายค้างหมด** → เต็มวง = 3 เสียง: **เปียโน** (ทำนอง+arp · front) · **เชลโลเบส** (re-bow ~3บีต · far) · **ไวโอลิน call-response** (far)
- **violinFill:** โน้ตทำนอง ≥2.5 บีต = "ช่องว่าง" → ไวโอลิน 3-โน้ต turn ย่าน 71–86 (เหนือทำนอง · ไม่ทับ · เข้าครึ่งหลังของช่องว่างให้พระเอกพูดก่อน) · gain 0.21
- **violinCounter (chorus only):** เส้นประสานเหนือทำนอง 74–86 · offbeat · **ตัด counter บนคอร์ดที่มี fill** · gain 0.14
- **section dynamics คงไว้** (verse×0.7→chorus×1.0 · section จริง) · **ตัด role-prominence duck/fill + master automation เดิม** (Option 1 แทน) · lead-driven (เปียโน/กีตาร์นำ) คงไว้
- **rebalance:** celloBus 0.30→0.16 · violinBus 0.13→0.62 (bowed punch เหนือเลข peak)
