...
1. **หน้าดู เปลี่ยนคีย์แล้วเสียงเปลี่ยนแบบ real-time** — **✅ dev แก้แล้ว (ข้าม domain ไป WT-A · แจ้ง SA) · พี่เอมยืนยัน ok**
   ...
   - *2 จุดที่แก้ใน `SongViewer.vue`:*
     (a) `startPlay` เดิมเล่นด้วย `content.key` (คีย์ต้นฉบับ) ไม่ใช่ `displayKey` → แก้เป็น `playSong({ ...resolved.value, key: displayKey.value }, …)` (เลือกคีย์ก่อนกดเล่น = ได้คีย์นั้น)
     (b) เพิ่ม `watch(displayKey, …)` — ถ้าเปลี่ยนคีย์ **ตอนกำลังเล่น** → เล่นซ้ำท่อนเดิมในคีย์ใหม่ทันที (นี่คือ "real-time" ที่หายไป)
   - *verify (deterministic):* unit test 2 เคส — เลือกคีย์ก่อนเล่น→ได้คีย์นั้น · หมุนคีย์ระหว่างเล่น→เล่นซ้ำคีย์ใหม่ · ผ่านทั้งคู่ (`SongViewer.play.test.js`)
   - **ไฟล์ที่แตะ = `src/components/SongViewer.vue` (domain ของ WT-A)** · เหตุผลที่แก้เอง: bug มีมาก่อน WT-0 (ไม่ใช่ regression จากการแยก surface) · WT-A ยังไม่เริ่ม (ไม่มี branch) ไม่ชนใคร · บล็อกพี่เอมตรวจงาน · **ขอ SA รับทราบ** ว่า WT-0 แตะ SongViewer และให้ WT-A rebase จากฐานที่มี fix นี้
