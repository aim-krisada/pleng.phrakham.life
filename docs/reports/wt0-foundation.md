...
1. **หน้าดู เปลี่ยนคีย์แล้วเสียงเปลี่ยนแบบ real-time** — **✅ dev แก้แล้ว (ข้าม domain ไป WT-A · แจ้ง SA) · พี่เอมยืนยัน ok**
   - *อาการจริง:* ผู้ใช้กด "ฟังเพลง" ก่อน แล้ว **หมุนคีย์ระหว่างเล่น** → เมื่อวานเสียงเปลี่ยนตามทันที วันนี้ไม่เปลี่ยน (ยืนยันด้วยป้ายดีบัก: ป้ายโชว์ "กำลังเล่นคีย์ E" ทั้งที่ตัวเลือกเป็น D)
   - *root cause จริง:* กลไก **live re-tune** (เปลี่ยนคีย์ตอนเล่นแล้วเสียงเปลี่ยนต่อจากจุดปัจจุบัน) หายตอน refactor `SongView` → `SongViewer` (commit `be35d26`) · ของเดิม (commit `4d5d49a`) เล่นที่คีย์ต้นฉบับแล้วขี่ transpose บน **detune ของแต่ละ oscillator** (100 cents = 1 semitone) · `setTranspose()` ปรับโน้ตที่ยังไม่ดังให้เป็นคีย์ใหม่ ณ จังหวะเริ่มของมัน — โน้ตที่กำลังดังจบในคีย์เดิม รอยต่อเนียน **ไม่ต้อง replay**
   - *fix = ดึงของเดิมกลับ (reference-first):*
     - `src/lib/midi.js` — คืน `keyTranspose()` · `setTranspose()` · `liveOscs`/`liveTranspose` · `playSong` รับ option `transpose` (default 0 = backward-compatible) · โน้ตขี่ transpose บน `osc.detune`
     - `src/components/SongViewer.vue` — เล่นด้วย `transpose: keyTranspose(content.key, displayKey)` · `watch(displayKey)` ตอนเล่น → `setTranspose(...)` (live re-tune ต่อจากจุดเดิม ไม่ restart) — ตรงกับที่พี่เอมชอบ
   - *ก่อนหน้านี้เคยแก้เป็น "restart ท่อนเดิมในคีย์ใหม่" — พี่เอมไม่ชอบเพราะเด้งกลับต้นเพลง จึงเปลี่ยนมาใช้ของเดิมที่เล่นต่อจากจุดปัจจุบัน*
   - *verify (deterministic):* `SongViewer.play.test.js` 2 เคส — เลือกคีย์ก่อนเล่น → `playSong` ได้ base คีย์เดิม + `transpose:+3` (E→G) · หมุนคีย์ระหว่างเล่น → `setTranspose(-2)` (E→D) **ไม่เรียก playSong ซ้ำ (ไม่ restart)** · ผ่านทั้งคู่ · ทั้งชุด 12/12
   - **ไฟล์ที่แตะ (domain ของ WT-A / core): `src/components/SongViewer.vue` + `src/lib/midi.js`** · เหตุผลที่แก้เอง: เป็น regression จาก refactor ก่อน WT-0 (ไม่ใช่จากการแยก surface) · คืนของเดิมที่ backward-compatible (transpose default 0 → EditorMode/ที่เรียก playSong เดิมไม่กระทบ) · WT-A ยังไม่เริ่ม ไม่ชนใคร · บล็อกพี่เอมตรวจงาน · **ขอ SA รับทราบ** ว่า WT-0 แตะ `SongViewer.vue` + `midi.js` และให้ WT-A rebase จากฐานที่มี fix นี้
   - *บทเรียน (ถึง SA เผื่อใส่ dev-sop):* bug ที่ "ได้ยิน" ต้องพิสูจน์ด้วยหู/ให้ผู้ทดสอบลอง หรือใส่ตัวชี้วัดบนจอ (เช่นป้าย "กำลังเล่นคีย์ X") **ก่อน** แก้ · ครั้งนี้ dev วินิจฉัยจาก "คอร์ดบนจอ" (ซึ่ง transpose อยู่แล้ว) เลยวนหลายรอบ — ถ้าใส่ป้ายดีบัก + ถาม repro (เปลี่ยนคีย์ก่อน/ระหว่างเล่น) ตั้งแต่แรกจะจบเร็วกว่ามาก
   - *latent bug ที่พบระหว่างทาง (ยังไม่แก้ → WT-A):* `watch(() => props.song)` รีเซ็ต `displayKey` กลับคีย์ต้นฉบับทุกครั้ง object เพลงเปลี่ยน — ปล่อยไว้ให้ WT-A
...
5. **หน้าดู ความเร็ว (tempo) ไม่เปลี่ยนตอนกำลังเล่น** — **→ SA จ่าย WT-A (feature ใหม่ ไม่ใช่ regression) · ไม่ใช่งานฐาน WT-0** · dev verify แล้ว: **tempo พื้นฐานทำงาน** (เลือก ♩=132 ก่อนกดเล่น → `playSong` ได้ `bpm:132` · unit test ผ่าน) · git ยืนยัน **ไม่เคยมี live-tempo** (มีแค่ commit `5748721` เพิ่ม selector เฉย ๆ — ต่างจาก key ที่ live re-tune เคยมีแล้วหาย) · ที่ต่างจาก key: การเปลี่ยน tempo สด ๆ ทำ "seamless" ไม่ได้เหมือน detune (ต้อง re-schedule โน้ตที่เหลือ = เล่นต่อจากจุดปัจจุบันในความเร็วใหม่) → เป็น **งานออกแบบ+สร้างของ WT-A** · **ข้อเสนอ dev:** เมื่อเปลี่ยน tempo ตอนเล่น → หยุดแล้วเล่นต่อจากบรรทัด/โน้ตปัจจุบันที่ bpm ใหม่ (analog ของ key แต่ไม่ seamless) · รอ SA เขียน US/DS ให้ WT-A
