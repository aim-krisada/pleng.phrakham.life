...
1. **หน้าดู เปลี่ยน key real-time (เสียงที่เล่น)** — **✅ dev แก้แล้ว (ข้าม domain ไป WT-A · แจ้ง SA)** · *แก้คำวินิจฉัยรอบแรก:* ทีแรกผมเช็คแค่ "คอร์ดบนจอ" ซึ่ง transpose ปกติ — แต่พี่เอมหมายถึง **เสียง playback** ที่ต้องเปลี่ยนคีย์ตาม · root cause: `SongViewer.startPlay` เรียก `playSong(resolved.value, …)` โดย `resolved.key = content.key` (คีย์ต้นฉบับ) ไม่ใช่ `displayKey` ที่ผู้ใช้เลือก → คอร์ดบนจอย้ายคีย์ แต่เสียงคาที่คีย์เดิม · fix: `playSong({ ...resolved.value, key: displayKey.value }, …)` · verify: `songToNotes` คีย์ E → midi 64, คีย์ G → midi 67 (เสียง transpose ตามคีย์แล้ว)
   - **ไฟล์ที่แตะ = `src/components/SongViewer.vue` (domain ของ WT-A)** · เหตุผลที่แก้เอง: เป็น bug ที่มีมาก่อน WT-0 (ไม่ใช่ regression จากการแยก surface) · WT-A ยังไม่เริ่ม (ไม่มี branch) → merge เข้าฐานก่อน ไม่ชนใคร · บล็อกพี่เอมตรวจงานอยู่ · **ขอ SA รับทราบ** ว่า WT-0 แตะ SongViewer 1 บรรทัด และให้ WT-A rebase จากฐานที่มี fix นี้แล้ว
   - *latent bug ที่พบระหว่างทาง (ยังไม่แก้ → WT-A):* `SongViewer` มี `watch(() => props.song)` รีเซ็ต `displayKey` กลับคีย์ต้นฉบับทุกครั้งที่ object เพลงเปลี่ยน — ถ้าผู้ใช้เลือกคีย์แล้วมีการแก้เพลง คีย์ที่เลือกถูกล้าง · ปล่อยไว้ให้ WT-A (ไม่ใช่สิ่งที่พี่เอมรายงาน)
