...
const PORT = 9481
const PROFILE = 'C:\\Users\\aimkr\\AppData\\Local\\Temp\\claude\\cdp-unverified-v1'
const APP = 'http://127.0.0.1:5497/'
const SHOTS = 'C:\\gl\\pm-inbox\\pleng\\unverified-v1-shots'
...
async function probe() {
  return cdp.eval(`(() => {
    ...
    return {
      ...
      count: chip ? (chip.querySelector('.rc-count')||{}).textContent : null,
      labelShown: chip ? getComputedStyle(chip.querySelector('.rc-label')).display !== 'none' : null,
...
log('DoD1 shot (landing):', await shot('01-v1-approver-desktop.png'))
log('DoD1 queue after click:', JSON.stringify(await clickChip()))
log('DoD1 shot (queue):', await shot('01b-v1-approver-queue-desktop.png'))
await load({ loggedIn: true, width: 360, height: 780 })
await clickChip()
log('DoD1 shot (queue 360):', await shot('01c-v1-approver-queue-360.png'))
await load({ loggedIn: true })
...
log('DoD2 shot:', await shot('02-v1-anon-desktop.png'))
...
for (const w of [320, 360, 390, 412]) {
  ...
  if (w === 360 || w === 390) { await cdp.send('Page.navigate', { url: APP }); await sleep(2000); log(`DoD5 shot:`, await shot(`05-v1-approver-${w}.png`)) }
}
// anon on mobile too
await load({ loggedIn: false, width: 360, height: 780 })
log('DoD2c anon/360:', JSON.stringify(await probe()))
log('DoD2c shot:', await shot('06-v1-anon-360.png'))
